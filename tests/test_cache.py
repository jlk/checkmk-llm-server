...
class TestCacheEntry:
    ...
    def test_cache_entry_expiration(self):
        ...
        entry = CacheEntry(
            key="test_key", value="test_value", expires_at=time.time() - 1
        )
...
class TestCachedHostService:
    ...
    async def test_cached_list_hosts(self, cached_host_service):
        ...
        assert (
            cached_host_service.checkmk.list_hosts.call_count == 1
        )  # No additional call
        assert result1 == result2
    ...
    async def test_cache_invalidation_on_create(self, cached_host_service):
        ...
        cached_host_service.checkmk.create_host = AsyncMock(
            return_value={"id": "new_host"}
        )
