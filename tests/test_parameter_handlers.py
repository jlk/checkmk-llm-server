...
class TestHandlerRegistry:
    ...
    def test_handlers_for_ruleset(self, registry):
        ...
        handlers = registry.get_handlers_for_ruleset("checkgroup_parameters:temperature")
        assert len(handlers) == 1
        assert handlers[0].name == "temperature"
...
class TestTemperatureParameterHandler:
    ...
    def test_parameter_suggestions(self, handler):
        ...
        cpu_suggestion = next((s for s in suggestions if s['parameter'] == 'levels'), None)
        if cpu_suggestion:
...
class TestCustomCheckParameterHandler:
    ...
    def test_command_validation(self, handler):
        ...
        valid_params = {
            "command_line": "check_disk -w 80% -c 90% /var",
            "timeout": 30
        }
...
class TestDatabaseParameterHandler:
    ...
    def test_connection_params_validation(self, handler):
        ...
        messages = handler._validate_connection_params(conn_params, ["hostname", "port", "database"])
...
class TestHandlerIntegration:
    ...
    def test_handler_chaining(self):
        ...
        handlers = registry.get_handlers_for_service("Custom Temperature Check", limit=3)
        ...
        if len(handlers) > 1:
            ...
            temp_idx = next((i for i, name in enumerate(handler_names) if name == "temperature"), -1)
            custom_idx = next((i for i, name in enumerate(handler_names) if name == "custom_checks"), -1)
