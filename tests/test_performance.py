                ...
                "value": [
                    {
                        ...
                        "attributes": {
                            "alias": f"Host {i}",
                            "description": f"This is host number {i} with lots of data"
                            * 10,
                        },
                    }
                    for i in range(offset, min(offset + limit, total_items))
...
class TestBatchPerformance:
    ...
    async def test_batch_processing_throughput(self):
        ...
        result = await processor.process_batch(
            items=items, operation=fast_operation, batch_id="throughput_test"
        )
...
class TestMetricsPerformance:
    ...
    async def test_metrics_collection_overhead(self):
        ...
        print(
            f"Baseline: {baseline_duration:.3f}s, With metrics: {metrics_duration:.3f}s"
        )
...
async def test_memory_leak_detection():
    ...
    for iteration in range(10):
        ...
        if iteration % 3 == 0:
            ...
            print(
                f"Iteration {iteration}: Memory usage {current_memory:.1f}MB (+{memory_growth:.1f}MB)"
            )
