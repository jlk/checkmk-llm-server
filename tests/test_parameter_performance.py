...
class PerformanceMetrics:
    ...
    def p95_operation_time(self) -> float:
        ...
        if self.operation_times:
            return statistics.quantiles(self.operation_times, n=20)[18] * 1000  # 95th percentile
        return 0
...
class TestHandlerRegistryPerformance:
    ...
    def test_concurrent_handler_access(self, registry_with_handlers):
        ...
        service_names = [
            "CPU Temperature", "MySQL Connections", "HTTP Check", "MRPE Script"
        ]
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [
                executor.submit(worker_task, worker_id, 100)
        ...
        assert summary["operations_per_second"] > 2000, \
            f"Concurrent handler selection too slow: {summary['operations_per_second']:.0f} ops/sec"
        assert summary["p95_operation_time_ms"] < 5.0, \
    ...
    def test_handler_caching_effectiveness(self, registry_with_handlers):
        ...
        assert speedup_ratio > 2.0, \
            f"Caching not effective enough: {speedup_ratio:.2f}x speedup"
...
class TestParameterGenerationPerformance:
    ...
    def test_parameter_generation_throughput(self, handlers):
        ...
        # Performance assertions
        assert summary["operations_per_second"] > 1000, \
            f"Parameter generation too slow: {summary['operations_per_second']:.0f} ops/sec"
        assert summary["average_operation_time_ms"] < 5.0, \
    ...
    def test_validation_performance(self, handlers):
        ...
        test_cases = [
            ("temperature", {"levels": (75.0, 85.0), "output_unit": "c"}, "CPU Temperature"),
            ("custom_checks", {"command_line": "check_disk -w 80% -c 90%", "timeout": 30}, "MRPE check_disk"),
            ("database", {"levels": (80.0, 90.0), "hostname": "db.example.com", "port": 3306}, "MySQL Connections"),
            ("network_services", {"url": "https://api.example.com/health", "response_time": (2.0, 5.0)}, "HTTPS API"),
        ]
        ...
        # Performance assertions
        assert summary["operations_per_second"] > 500, \
            f"Parameter validation too slow: {summary['operations_per_second']:.0f} ops/sec"
        assert summary["average_operation_time_ms"] < 10.0, \
    ...
    def test_memory_efficiency_large_dataset(self, handlers):
        ...
        assert summary["memory_delta_mb"] < 100, \
            f"Memory usage too high for large dataset: {summary['memory_delta_mb']:.1f}MB"
        assert summary["operations_per_second"] > 800, \
...
class TestParameterServicePerformance:
    ...
    def mock_checkmk_client(self):
        ...
        client.get_effective_parameters = AsyncMock(side_effect=mock_get_effective_parameters)
    ...
    async def test_parameter_service_throughput(self, mock_checkmk_client, mock_config):
        ...
        assert summary["operations_per_second"] > 50, \
            f"Parameter service too slow: {summary['operations_per_second']:.0f} ops/sec"
        assert summary["average_operation_time_ms"] < 100, \
            f"Average operation time too high: {summary['average_operation_time_ms']:.2f}ms"
    ...
    async def test_bulk_parameter_operations(self, mock_checkmk_client, mock_config):
        ...
        async def process_operation(operation):
            async with semaphore:
                op_start = time.perf_counter()
                result = await service.get_specialized_defaults(operation["service_name"])
                op_end = time.perf_counter()
        ...
        assert summary["operations_per_second"] > 30, \
...
class TestScalabilityBenchmarks:
        ...
        for num_handlers in [10, 50, 100, 500]:
            ...
            for i in range(num_handlers):
                class TestHandler(BaseParameterHandler):
                    def __init__(self):
                        super().__init__(f"handler_{i}")
                        self.service_patterns = [f"service_{i}"]
                        self.supported_rulesets = [f"ruleset_{i}"]
                    
                    def get_default_parameters(self, service_name, context=None):
                        return HandlerResult(success=True, parameters={"id": i})
    ...
    def test_memory_usage_scalability(self):
        ...
        for count, memory_delta in memory_usage:
            ...
            assert memory_per_service < 0.1, \
                f"Memory usage too high: {memory_per_service:.3f}MB per service"
        
        print("Memory Usage Scalability:")
        for count, memory_delta in memory_usage:
            print(f"  {count} services: {memory_delta:.1f}MB total, {memory_delta/count:.3f}MB per service")
