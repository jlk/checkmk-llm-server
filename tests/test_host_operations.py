...
def app_config():
    ...
    return AppConfig(
        checkmk=CheckmkConfig(
            server_url="https://test.com", username="test", password="test", site="test"
        ),
        llm=LLMConfig(),
        default_folder="/test",
        log_level="INFO",
...
class TestHostOperationsManager:
    ...
    def test_initialization(self, mock_checkmk_client, mock_llm_client, app_config):
        ...
        manager = HostOperationsManager(
            mock_checkmk_client, mock_llm_client, app_config
        )
    ...
    def test_process_command_success(
        self, host_manager, mock_llm_client, mock_checkmk_client
    ):
        """Test successful command processing."""
        # Setup mocks
        parsed_command = ParsedCommand(
            operation=HostOperation.LIST, parameters={}, confidence=0.9
        )
    ...
    def test_process_command_error(
        ...
        mock_llm_client.parse_command.return_value = parsed_command
        mock_checkmk_client.get_host.side_effect = CheckmkAPIError(
            "Host not found", status_code=404
        )
    ...
    def test_process_command_syntax_error(
        self, host_manager, mock_llm_client, mock_checkmk_client
    ):
    ...
    def test_list_hosts_basic(self, host_manager, mock_checkmk_client):
        ...
        mock_checkmk_client.list_hosts.assert_called_once_with(
    ...
    def test_list_hosts_with_search(self, host_manager, mock_checkmk_client):
        ...
        mock_checkmk_client.list_hosts.return_value = [
            {
                "id": "web01",
                "extensions": {"folder": "/web", "attributes": {"alias": "Web Server"}},
            },
            {
                "id": "db01",
                "extensions": {
                    "folder": "/database",
                    "attributes": {"alias": "Database"},
                },
            },
            {
                "id": "web02",
    ...
    def test_create_host_basic(self, host_manager, mock_checkmk_client):
        ...
        result = host_manager._create_host({"host_name": "new-host", "folder": "/test"})

        assert result["id"] == "new-host"
        mock_checkmk_client.create_host.assert_called_once_with(
            folder="/test", host_name="new-host", attributes={}, bake_agent=False
        )
    ...
    def test_get_host_success(self, host_manager, mock_checkmk_client):
        ...
        mock_checkmk_client.get_host.assert_called_once_with(
            "test-host", effective_attributes=False
        )

    def test_get_host_with_effective_attributes(
        self, host_manager, mock_checkmk_client
    ):
    ...
    def test_update_host_success(self, host_manager, mock_checkmk_client):
        ...
        result = host_manager._update_host(
            {"host_name": "test-host", "attributes": {"ipaddress": "192.168.1.20"}}
        )

        assert result["id"] == "test-host"
        mock_checkmk_client.update_host.assert_called_once_with(
            "test-host", {"ipaddress": "192.168.1.20"}, etag=None
        )
    ...
    def test_execute_operation_unsupported(self, host_manager):
        ...
        command = ParsedCommand(
            operation="unsupported_operation", parameters={}  # Invalid operation
        )
    ...
    def test_interactive_create_host_success(
        self, mock_input, host_manager, mock_checkmk_client
    ):
