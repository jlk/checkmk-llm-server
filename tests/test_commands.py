...
from checkmk_agent.commands.base import (
    BaseCommand, CommandContext, CommandResult, CommandCategory
)
...
from checkmk_agent.commands.service_commands import (
    ListServicesCommand, GetServiceStatusCommand, AcknowledgeServiceCommand
)
...
class TestBaseCommand:
    ...
    def test_command_context_creation(self):
        ...
        with pytest.raises(ValueError, match="Required parameter 'missing' not provided"):
            context.require_parameter("missing")
    
    def test_command_result_creation(self):
        """Test CommandResult creation and methods."""
        # Success result
        success_result = CommandResult.success_result(data={"key": "value"}, message="Success!")
        ...
        # Error result
        error_result = CommandResult.error_result("Something went wrong", data={"error_code": 500})
        assert error_result.success is False
        assert error_result.error == "Something went wrong"
        assert error_result.data == {"error_code": 500}
        
        # Metadata
        result = CommandResult.success_result().with_metadata(timestamp=datetime.now(), source="test")
...
class TestCommandRegistry:
    ...
    def test_register_command_conflicts(self, registry, mock_command):
        ...
        with pytest.raises(ValueError, match="Command 'test_command' is already registered"):
            registry.register(mock_command)
        ...
        with pytest.raises(ValueError, match="Alias 'test_command' conflicts with existing command name"):
...
class TestServiceCommands:
    ...
    def test_get_service_status_command(self, mock_checkmk_client):
        ...
        context = CommandContext("get status", {"host_name": "server01"})  # Missing service_description
        validation_result = command.validate(context)
