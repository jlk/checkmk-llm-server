...
class TestCheckMkClientStatusMethods:
    ...
    def test_get_service_health_summary(self, mock_request):
        ...
        mock_response = {
            'value': [
                {'extensions': {'state': 0, 'acknowledged': 0, 'scheduled_downtime_depth': 0}},
                {'extensions': {'state': 0, 'acknowledged': 0, 'scheduled_downtime_depth': 0}},
                {'extensions': {'state': 1, 'acknowledged': 0, 'scheduled_downtime_depth': 0}},
                {'extensions': {'state': 2, 'acknowledged': 1, 'scheduled_downtime_depth': 0}},
                {'extensions': {'state': 3, 'acknowledged': 0, 'scheduled_downtime_depth': 1}}
            ]
        }
        mock_request.return_value = mock_response
    ...
    def test_service_status_integration_workflow(self):
        ...
        with patch.object(self.client, '_make_request') as mock_request:
            ...
            health_response = {
                'value': [
                    {'extensions': {'state': 0, 'acknowledged': 0, 'scheduled_downtime_depth': 0}},
                    {'extensions': {'state': 2, 'acknowledged': 0, 'scheduled_downtime_depth': 0}},
                    {'extensions': {'state': 1, 'acknowledged': 1, 'scheduled_downtime_depth': 0}}
                ]
            }
            
            problems_response = {
                'value': [
                    {'extensions': {'host_name': 'server01', 'description': 'Critical Service', 'state': 2}},
                    {'extensions': {'host_name': 'server02', 'description': 'Warning Service', 'state': 1}}
                ]
            }
            ...
            ack_response = {
                'value': [
                    {'extensions': {'host_name': 'server02', 'description': 'Warning Service', 'acknowledged': 1}}
                ]
            }
            
            # Set up mock to return different responses for different calls
            mock_request.side_effect = [health_response, problems_response, critical_response, ack_response]
...
class TestCheckMkClientStatusIntegration:
    ...
    def test_complete_status_monitoring_scenario(self, mock_request):
        ...
        mock_request.return_value = {'value': [s for s in services_data if s['extensions']['state'] != 0]}
        problems = self.client.list_problem_services()
        assert len(problems) == 20
        
        # Test critical services only
        mock_request.return_value = {'value': [s for s in services_data if s['extensions']['state'] == 2]}
        critical = self.client.get_services_by_state(2)
        assert len(critical) == 10
        
        # Test acknowledged services
        mock_request.return_value = {'value': [s for s in services_data if s['extensions']['acknowledged'] == 1]}
        acknowledged = self.client.get_acknowledged_services()
        ack_count = len([s for s in services_data if s['extensions']['acknowledged'] == 1])
        assert len(acknowledged) == ack_count
        
        # Test services in downtime
        mock_request.return_value = {'value': [s for s in services_data if s['extensions']['scheduled_downtime_depth'] > 0]}
        downtime = self.client.get_services_in_downtime()
        downtime_count = len([s for s in services_data if s['extensions']['scheduled_downtime_depth'] > 0])
