...
from checkmk_agent.services.batch import (
    BatchProcessor, BatchItem, BatchProgress, BatchResult, BatchItemStatus,
...
class TestBatchProcessor:
    ...
    async def test_process_batch_with_validation(self, batch_processor):
        ...
        skipped_items = [item for item in result.items if item.status == BatchItemStatus.SKIPPED]
...
async def test_batch_error_resilience():
    ...
    result = await processor.process_batch(
        items=items,
        operation=unpredictable_operation,
        batch_id="resilience_test"
    )
