...
class TestMCPParameterTools:
    ...
    def mock_checkmk_client(self):
        ...
        client.update_rule = AsyncMock(return_value={
            "result": {"rule_id": "param_rule_001", "updated": True}
        })
        ...
        client.search_services = AsyncMock(return_value={
            "result": [
    ...
    def mcp_server(self, mock_checkmk_client, mock_config):
        ...
        # Initialize services
        from checkmk_agent.services import HostService, StatusService, ServiceService, ParameterService
    ...
    async def test_create_specialized_rule_tool(self, mcp_server):
        ...
        assert result_data["data"]["handler_used"] == "unknown"  # Updated to match actual response
...
class TestMCPToolErrorHandling:
    ...
    async def test_tool_error_handling(self, mcp_server_with_errors):
        ...
        result = await mcp_server_with_errors.call_tool("get_specialized_defaults", arguments)
        result_data = json.loads(result['content'][0]['text'])
        ...
        result = await mcp_server_with_errors.call_tool("create_specialized_rule", arguments)
        result_data = json.loads(result['content'][0]['text'])
    ...
    async def test_malformed_parameters_handling(self, mcp_server_with_errors):
        ...
        result = await mcp_server_with_errors.call_tool("validate_specialized_parameters", arguments)
        result_data = json.loads(result['content'][0]['text'])
...
class TestMCPToolPerformance:
    ...
    async def test_concurrent_tool_calls(self, fast_mcp_server):
        ...
        for i in range(50):
            tool_calls.append(
                fast_mcp_server.call_tool("get_specialized_defaults", 
                                        {"service_name": f"CPU {i} Temperature"})
            )
        
        for i in range(25):
            tool_calls.append(
                fast_mcp_server.call_tool("validate_specialized_parameters", {
        ...
        for i in range(25):
            tool_calls.append(
                fast_mcp_server.call_tool("get_parameter_suggestions", {
    ...
    async def test_bulk_operations_performance(self, fast_mcp_server):
        ...
        assert per_service_time < 0.02, f"Per-service time too high: {per_service_time:.3f}s"
