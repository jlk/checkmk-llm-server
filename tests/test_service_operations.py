...
class TestServiceOperationsManager:
    ...
    def service_manager(self, mock_checkmk_client, mock_llm_client, mock_config):
        ...
        return ServiceOperationsManager(
            mock_checkmk_client, mock_llm_client, mock_config
        )
    ...
    def test_list_host_services(self, service_manager, mock_checkmk_client):
        ...
        service_manager.llm_client.chat_completion.return_value = json.dumps(
            {"action": "list_services", "parameters": {"host_name": "server01"}}
        )
        ...
        mock_checkmk_client.list_host_services.assert_called_once_with(
            "server01", columns=["description", "state", "plugin_output"]
        )
    ...
    def test_list_all_services(self, service_manager, mock_checkmk_client):
        ...
        # Mock LLM response
        service_manager.llm_client.chat_completion.return_value = json.dumps(
            {"action": "list_services", "parameters": {}}
        )
        ...
        mock_checkmk_client.list_all_services.assert_called_once_with(
    ...
    def test_create_service_downtime(self, service_manager, mock_checkmk_client):
        ...
        with patch("checkmk_agent.service_operations.datetime") as mock_datetime:
            ...
            result = service_manager.process_command(
    ...
    def test_discover_services(self, service_manager, mock_checkmk_client):
        ...
        mock_discovery_result = {
            "extensions": {
                ...
                "vanished": [{"service_description": "Old Service"}],
                "ignored": [],
            }
        }
        mock_checkmk_client.get_service_discovery_result.return_value = (
            mock_discovery_result
        )

        # Mock LLM response
        service_manager.llm_client.chat_completion.return_value = json.dumps(
            {"action": "discover_services", "parameters": {"host_name": "server01"}}
        )
        ...
        mock_checkmk_client.start_service_discovery.assert_called_once_with(
            "server01", "refresh"
        )
        mock_checkmk_client.get_service_discovery_result.assert_called_once_with(
            "server01"
        )
    ...
    def test_get_service_statistics(self, service_manager, mock_checkmk_client):
        ...
        # Mock response
        mock_services = [
            {"extensions": {"state": "OK", "host_name": "server01"}},
            {"extensions": {"state": "WARN", "host_name": "server01"}},
            {"extensions": {"state": "CRIT", "host_name": "server02"}},
        ]
        mock_checkmk_client.list_all_services.return_value = mock_services
    ...
    def test_test_connection_failure(self, service_manager, mock_checkmk_client):
        ...
        mock_checkmk_client.list_all_services.side_effect = CheckmkAPIError(
            "Connection failed"
        )
    ...
    def test_get_instructions_acknowledge_service(
        self, service_manager, mock_checkmk_client
    ):
        ...
        service_manager.llm_client.chat_completion.return_value = json.dumps(
            {
                "action": "get_instructions",
                "parameters": {"instruction_type": "acknowledge_service"},
            }
        )
    ...
    def test_get_instructions_general(self, service_manager, mock_checkmk_client):
        ...
        service_manager.llm_client.chat_completion.return_value = json.dumps(
            {
                "action": "get_instructions",
                "parameters": {"instruction_type": "general"},
            }
        )
...
class TestServiceOperationsIntegration:
    ...
    def mock_checkmk_client(self):
        ...
        client.get_service_discovery_result.return_value = {
            "extensions": {
                ...
                "vanished": [{"service_description": "Old Process"}],
                "ignored": [{"service_description": "Temp sensor"}],
            }
        }
    ...
    def mock_llm_client(self):
        ...
        def mock_chat_completion(prompt):
            ...
            if '"list services for server01"' in prompt:
                ...
                '"create 4 hour downtime for Memory utilization on server01"' in prompt
            ):
                return json.dumps(
                    {
                        "action": "create_downtime",
                ...
                return json.dumps(
                    {
                        ...
                        "parameters": {"host_name": "server01", "mode": "refresh"},
                    }
                )
            else:
                # Default to list_services if we can't match
                return json.dumps({"action": "list_services", "parameters": {}})
    ...
    def test_service_manager_initialization(
        self, mock_checkmk_client, mock_llm_client, mock_config
    ):
        """Test that ServiceOperationsManager initializes correctly."""
        manager = ServiceOperationsManager(
            mock_checkmk_client, mock_llm_client, mock_config
        )
    ...
    def test_service_operations_api_calls(
        self, mock_checkmk_client, mock_llm_client, mock_config
    ):
