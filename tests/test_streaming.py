...
from checkmk_agent.services.streaming import (
    StreamingMixin, StreamBatch, StreamingHostService, StreamingServiceService
)
...
class TestStreamingServiceService:
    ...
    async def test_list_all_services_streamed(self, streaming_service_service):
        ...
        # Test streaming
        batches = []
        async for batch in streaming_service_service.list_all_services_streamed(batch_size=2):
            batches.append(batch)
    ...
    async def test_list_services_streamed_with_filter(self, streaming_service_service):
        ...
        async for batch in streaming_service_service.list_all_services_streamed(
            batch_size=10,
            state_filter=[ServiceState.WARNING, ServiceState.CRITICAL]
        ):
            batches.append(batch)
            if not batch.has_more:
                break
