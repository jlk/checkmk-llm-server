...
def retry_on_failure(
    max_retries: int = 3, delay: float = 1.0, backoff_multiplier: float = 2.0
):
    """
...
def check_connection_health(base_url: str, timeout: int = 10) -> Dict[str, Any]:
    ...
    try:
        ...
        response = requests.get(
            test_url, timeout=timeout, verify=False
        ...
        if response_time_ms > 5000:
            health_info["suggestions"].append(
                "Server response time is slow (>5s). Check network connectivity."
            )
        elif response_time_ms > 2000:
            health_info["suggestions"].append(
                "Server response time is elevated (>2s). Monitor server performance."
            )
    ...
    if isinstance(response_data, dict):
        ...
        if expected_fields and response_data:
            ...
            if isinstance(first_item, dict):
                ...
                if missing_fields:
                    raise ValueError(
                        f"{response_type} items missing required fields: {', '.join(missing_fields)}"
                    )
...
def validate_service_data(service_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if missing_fields:
        ...
        logger.warning(
            f"Service data missing expected fields: {', '.join(missing_fields)}"
        )
