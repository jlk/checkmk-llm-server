...
class ParsedCommand:
    ...
    def __repr__(self):
        return (
            f"ParsedCommand(operation={self.operation}, parameters={self.parameters})"
        )
...
- Offline: {'Yes' if extensions.get('is_offline') else 'No'}"""
            ...
            if isinstance(data, list):
                ...
                for rule in data:
                    ...
                    rule_list.append(
                        f"- {rule_id} (ruleset: {ruleset}, folder: {folder})"
                    )
...
"""
        ...
        try:
            ...
            if not hasattr(content_block, "text"):
                raise ValueError(
                    "Anthropic response content block has no text attribute"
                )
...
def create_llm_client(
    config: LLMConfig, provider: Optional[LLMProvider] = None
) -> LLMClient:
