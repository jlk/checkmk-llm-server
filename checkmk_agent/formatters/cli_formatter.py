...
class CLIFormatter(BaseFormatter):
    ...
    def format_health_dashboard(self, data: Dict[str, Any]) -> str:
        ...
        if service_states:
            ...
            lines.append(f"  ✅ OK: {self.ui.colorize(str(ok_count), 'success')} services")
            if warning_count > 0:
                lines.append(f"  ⚠️  WARNING: {self.ui.colorize(str(warning_count), 'warning')} services")
            if critical_count > 0:
                lines.append(f"  ❌ CRITICAL: {self.ui.colorize(str(critical_count), 'error')} services")
            if unknown_count > 0:
                lines.append(f"  ❓ UNKNOWN: {self.ui.colorize(str(unknown_count), 'info')} services")
        ...
        if problem_summary:
            ...
            if total_problems > 0:
                ...
                lines.append(f"  🔴 {urgent_problems} urgent problem{'s' if urgent_problems != 1 else ''} require immediate attention")
                lines.append(f"  💡 {unacknowledged} unacknowledged problem{'s' if unacknowledged != 1 else ''} need review")
            else:
                lines.append("")
                lines.append(self.ui.format_message("✅ No problems detected!", MessageType.SUCCESS))
        
        # Critical problems
        critical_problems = data.get('critical_problems', [])
        if critical_problems:
            lines.append("")
            lines.append(self.ui.colorize(f"🔴 Critical Services ({len(critical_problems)}):", "error"))
            for problem in critical_problems[:5]:  # Show top 5
                lines.append(f"  ❌ {problem.get('host_name', '')}/{problem.get('service_name', '')}")
    ...
    def format_service_status(self, data: Dict[str, Any]) -> str:
        ...
        if not service:
            return self.ui.format_message("No service data available", MessageType.ERROR)
        ...
        lines.append(self.ui.colorize(f"📋 Service Status: {host_name}/{service_name}", "header"))
        ...
        if service.get('acknowledged'):
            lines.append(f"🔕 Acknowledged by {service.get('acknowledgement_author', 'unknown')}")
        ...
        if related_services:
            lines.append("")
            lines.append(self.ui.colorize(f"Related Services on {host_name}:", "header"))
    ...
    def _format_service_stats(self, stats: Dict[str, int]) -> str:
        ...
        if unhandled > 0:
            lines.append(f"  🚨 Unhandled Problems: {self.ui.colorize(str(unhandled), 'error')}")
