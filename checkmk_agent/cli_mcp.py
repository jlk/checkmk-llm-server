...
class MCPCLIContext:
    ...
    def __init__(self, config: AppConfig, mcp_client: CheckmkMCPClient, formatter: CLIFormatter):
        self.config = config
...
async def create_host(ctx_obj: MCPCLIContext, name, folder, ip, alias, tags):
    ...
    try:
        ...
        if result.get('success'):
            click.echo(ctx_obj.formatter.format_success(
                f"Successfully created host '{name}'"
            ))
            if result.get('data'):
...
async def show_host(ctx_obj: MCPCLIContext, name, status):
    ...
    try:
        result = await ctx_obj.mcp_client.get_host(
            name=name,
            include_status=status
        )
...
@click.pass_obj
@async_command
async def update_host(ctx_obj: MCPCLIContext, name, folder, ip, alias, add_tag, remove_tag):
    ...
    try:
        ...
        if add_tag or remove_tag:
            ...
            click.echo(ctx_obj.formatter.format_warning(
        ...
        if result.get('success'):
            click.echo(ctx_obj.formatter.format_success(
                f"Successfully updated host '{name}'"
            ))
            if result.get('data'):
                click.echo(ctx_obj.formatter.format_host_details(result['data']))
        else:
...
async def delete_host(ctx_obj: MCPCLIContext, name, force):
    ...
    try:
        ...
        if result.get('success'):
            click.echo(ctx_obj.formatter.format_success(
                f"Successfully deleted host '{name}'"
            ))
        else:
            click.echo(ctx_obj.formatter.format_error(
...
@click.option('--state', '-s', 
              ...
              multiple=True, help='Filter by service state')
...
@click.argument('host_name')
@click.argument('service_name')
@click.option('--comment', '-c', default='Acknowledged via CLI', help='Acknowledgement comment')
...
@click.option('--persistent/--no-persistent', default=False, help='Persist across restarts')
@click.pass_obj
@async_command
async def acknowledge_problem(ctx_obj: MCPCLIContext, host_name, service_name, comment, sticky, notify, persistent):
...
@click.option('--comment', '-c', default='Scheduled downtime via CLI', help='Downtime comment')
...
@click.pass_obj
@async_command
async def create_downtime(ctx_obj: MCPCLIContext, host_name, service_name, duration, comment, start, fixed):
...
@click.option('--mode', '-m', 
              ...
              default='refresh', help='Discovery mode')
@click.pass_obj
@async_command
async def discover_services(ctx_obj: MCPCLIContext, host_name, mode):
    ...
    try:
        ...
        if result.get('success'):
            click.echo(ctx_obj.formatter.format_success(result.get('message', 'Discovery completed')))
            if result.get('data'):
...
@click.pass_obj
@async_command
async def health_dashboard(ctx_obj: MCPCLIContext, problems_only, critical_only, host_filter):
...
@click.pass_obj
@async_command
async def analyze_host(ctx_obj: MCPCLIContext, host_name, grade, recommendations, compare):
...
def resources(ctx_obj: MCPCLIContext):
    ...
    resources = [
        ...
        ("checkmk://dashboard/problems", "Current critical problems across infrastructure"),
...
def prompts(ctx_obj: MCPCLIContext):
    ...
    prompts = [
        ("analyze_host_health", "Analyze the health of a specific host with detailed recommendations"),
        ("troubleshoot_service", "Comprehensive troubleshooting analysis for a service problem"),
        ("infrastructure_overview", "Get a comprehensive overview of infrastructure health and trends"),
        ("optimize_parameters", "Get parameter optimization recommendations for a service")
    ]
