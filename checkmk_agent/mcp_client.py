...
class CheckmkMCPClient:
    ...
    async def connect(self, server_path: Optional[str] = None, config_file: Optional[str] = None) -> None:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError("MCP session initialization timed out (known issue with MCP SDK 1.12.0 on macOS)")
                ...
                for content in result.contents:
                    if hasattr(content, 'text'):
                        try:
                            ...
                            return {"error": f"Invalid JSON in resource: {content.text}"}
    ...
    async def get_host_analysis_prompt(self, host_name: str, include_grade: bool = True) -> str:
        """Get AI prompt for host health analysis."""
        return await self.get_prompt("analyze_host_health", {
            "host_name": host_name,
            "include_grade": str(include_grade).lower()
        })
    
    async def get_service_troubleshooting_prompt(self, host_name: str, service_name: str) -> str:
        """Get AI prompt for service troubleshooting."""
        return await self.get_prompt("troubleshoot_service", {
            "host_name": host_name,
            "service_name": service_name
        })
    
    async def get_infrastructure_overview_prompt(self, time_range_hours: int = 24) -> str:
        """Get AI prompt for infrastructure overview."""
        return await self.get_prompt("infrastructure_overview", {
            "time_range_hours": str(time_range_hours)
        })
    
    async def get_parameter_optimization_prompt(self, host_name: str, service_name: str) -> str:
        """Get AI prompt for parameter optimization."""
        return await self.get_prompt("optimize_parameters", {
            "host_name": host_name,
            "service_name": service_name
        })
...
def create_mcp_client(config: AppConfig, config_file: Optional[str] = None) -> AsyncContextManager:
