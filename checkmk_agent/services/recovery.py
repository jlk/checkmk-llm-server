        ...
        for attempt in range(1, self.max_retries + 2):  # +1 for initial attempt
            try:
                ...
                if attempt <= self.max_retries:
                    ...
                    await get_metrics_collector().increment_counter(
                        f"retry.{operation_name}.attempt", attempt=str(attempt)
                    )
...
class FallbackHandler:
    ...
    async def execute_with_fallback(
        ...
        try:
            ...
            await get_metrics_collector().increment_counter(
                f"fallback.{operation_name}.triggered"
            )
            ...
            if not fallback_func:
                ...
                await get_metrics_collector().increment_counter(
                    f"fallback.{operation_name}.missing"
                )
                raise
            ...
            try:
                ...
                result = await fallback_func(
                    *(fallback_args or ()), **(fallback_kwargs or {})
                )
                await get_metrics_collector().increment_counter(
                    f"fallback.{operation_name}.success"
                )
                return result
                ...
                await get_metrics_collector().increment_counter(
                    f"fallback.{operation_name}.failed"
                )
                raise
...
class RecoveryMixin:
            ...
            async def wrapper(*args, **func_kwargs):
                ...
                async def execute():
                    if retry_policy:
                        retry = self.get_retry_policy(
                            f"{name}_retry", **kwargs.get("retry_config", {})
                        )
                        return await retry.execute(
                            lambda: func(*args, **func_kwargs), operation_name=name
                        )
                    else:
                ...
                if circuit_breaker:
                    cb = self.get_circuit_breaker(
                        f"{name}_circuit", **kwargs.get("circuit_config", {})
                    )
    ...
    async def execute_with_recovery(
        ...
        if enable_circuit_breaker:
            ...
            if enable_fallback:
                return await self.fallback_handler.execute_with_fallback(
                    execute, operation_name, fallback_args, fallback_kwargs
                )
            else:
                return await execute()
