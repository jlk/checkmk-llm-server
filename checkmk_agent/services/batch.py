...
class BatchProcessor:
    ...
    async def process_batch(
        ...
        # Create batch items
        batch_items = [
            BatchItem(id=f"{batch_id}_{i}", data=item) for i, item in enumerate(items)
        ]
            ...
            async with self._semaphore:
                try:
                    ...
                    for attempt in range(self.max_retries):
                        try:
                            ...
                            if attempt < self.max_retries - 1:
                                ...
                                progress.failed += 1
                                self.logger.error(
                                    f"Failed item {batch_item.id} after {self.max_retries} attempts: {e}"
                                )
                    ...
                    self.logger.exception(
                        f"Unexpected error processing item {batch_item.id}"
                    )
        ...
        await asyncio.gather(
            *[process_item(item) for item in batch_items], return_exceptions=True
        )
...
class BatchOperationsMixin:
    ...
    async def batch_update(
        ...
        try:
            self.logger.info(
                f"Starting batch update for {len(updates)} {resource_type}s"
            )
