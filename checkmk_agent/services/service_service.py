...
from .models.services import (
    ServiceInfo, ServiceListResult, ServiceStatusResult, ServiceAcknowledgeResult,
    ServiceDowntimeResult, ServiceDiscoveryResult, ServiceParameterResult, ServiceState
)
...
class ServiceService(BaseService):
    ...
    async def list_host_services(
        ...
        async def _list_services_operation():
            # Validate host name
            validation_errors = self._validate_required_params({"host_name": host_name}, ["host_name"])
            ...
            for service_data in services_data:
                service_info = self._convert_api_service_to_model(service_data, include_details)
            ...
            return ServiceListResult(
                ...
                state_filter=state_filter[0] if state_filter and len(state_filter) == 1 else None,
                stats=stats,
                metadata={
        ...
        return await self._execute_with_error_handling(_list_services_operation, f"list_host_services_{host_name}")
    ...
    async def get_service_status(
        ...
        async def _get_status_operation():
            ...
            # Get service status
            service_data = await self.checkmk.get_service_status(host_name, service_name)
            service_info = self._convert_api_service_to_model(service_data, include_details=True)
            ...
            if include_related:
                try:
                    ...
                    self.logger.warning(f"Could not get related services for {host_name}: {e}")
        ...
        return await self._execute_with_error_handling(_get_status_operation, f"get_service_status_{host_name}_{service_name}")
    ...
    async def acknowledge_problem(
        ...
        return await self._execute_with_error_handling(_acknowledge_operation, f"acknowledge_{host_name}_{service_name}")
    ...
    async def create_downtime(
        ...
        async def _create_downtime_operation():
            # Validate parameters
            validation_errors = self._validate_required_params(
                {"host_name": host_name, "service_name": service_name, "duration_hours": duration_hours},
        ...
        return await self._execute_with_error_handling(_create_downtime_operation, f"create_downtime_{host_name}_{service_name}")
    ...
    async def discover_services(
        ...
        async def _discovery_operation():
            ...
            if mode not in valid_modes:
                raise ValueError(f"Invalid discovery mode: {mode}. Valid modes: {valid_modes}")
            
            # Perform discovery via API
            discovery_result = await self.checkmk.start_service_discovery(host_name, mode)
        ...
        return await self._execute_with_error_handling(_discovery_operation, f"discover_services_{host_name}")
    ...
    async def list_all_services(
        ...
        return await self._execute_with_error_handling(_list_all_services_operation, "list_all_services")
    ...
    def _convert_api_service_to_model(self, service_data: Dict[str, Any], include_details: bool = False) -> ServiceInfo:
        ...
        if include_details:
            service_info.last_ok = self._safe_timestamp_conversion(service_data.get("last_time_ok"))
            service_info.next_check = self._safe_timestamp_conversion(service_data.get("next_check"))
            service_info.acknowledgement_author = service_data.get("acknowledgement_author")
            service_info.acknowledgement_comment = service_data.get("acknowledgement_comment")
            service_info.acknowledgement_time = self._safe_timestamp_conversion(service_data.get("acknowledgement_time"))
            service_info.downtime_start = self._safe_timestamp_conversion(service_data.get("downtime_start"))
            service_info.downtime_end = self._safe_timestamp_conversion(service_data.get("downtime_end"))
            ...
            service_info.notifications_enabled = service_data.get("notifications_enabled", True)
            service_info.active_checks_enabled = service_data.get("active_checks_enabled", True)
            service_info.passive_checks_enabled = service_data.get("passive_checks_enabled", True)
    ...
    def _calculate_service_stats(self, services: List[ServiceInfo]) -> Dict[str, int]:
        ...
        for service in services:
            ...
            if (service.state != ServiceState.OK and 
                not service.acknowledged and 
                not service.in_downtime):
