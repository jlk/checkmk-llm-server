...
class ServiceInfo(BaseModel):
    ...
    long_plugin_output: Optional[str] = Field(None, description="Extended service output")
    ...
    # Problem management
    acknowledged: bool = Field(False, description="Whether service problem is acknowledged")
    acknowledgement_author: Optional[str] = Field(None, description="Who acknowledged the problem")
    acknowledgement_comment: Optional[str] = Field(None, description="Acknowledgement comment")
    acknowledgement_time: Optional[datetime] = Field(None, description="When problem was acknowledged")
    
    # Downtime information
    in_downtime: bool = Field(False, description="Whether service is in scheduled downtime")
    ...
    max_check_attempts: Optional[int] = Field(None, description="Maximum check attempts")
    ...
    active_checks_enabled: bool = Field(True, description="Whether active checks are enabled")
    passive_checks_enabled: bool = Field(True, description="Whether passive checks are enabled")
...
class ServiceListResult(BaseModel):
    ...
    state_filter: Optional[ServiceState] = Field(None, description="State filter applied")
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Additional metadata")
    
    # Summary statistics
    stats: Dict[str, int] = Field(default_factory=dict, description="Service state statistics")
...
class ServiceStatusResult(BaseModel):
    ...
    # Additional context
    related_services: List[ServiceInfo] = Field(default_factory=list, description="Related services on same host")
...
class ServiceAcknowledgeResult(BaseModel):
    ...
    persistent: bool = Field(description="Whether acknowledgement persists across restarts")
...
class ServiceDiscoveryResult(BaseModel):
    ...
    # Discovery results
    new_services: List[str] = Field(default_factory=list, description="Newly discovered services")
    removed_services: List[str] = Field(default_factory=list, description="Services that vanished")
    changed_services: List[str] = Field(default_factory=list, description="Services with changed parameters")
    unchanged_services: List[str] = Field(default_factory=list, description="Unchanged services")
...
class ServiceParameterResult(BaseModel):
    ...
    rule_id: Optional[str] = Field(None, description="Rule ID if parameter rule was created")
    ruleset: Optional[str] = Field(None, description="Ruleset name")
    effective_parameters: Dict[str, Any] = Field(default_factory=dict, description="Effective parameters after change")
    
    # Change tracking
    changes_made: List[str] = Field(default_factory=list, description="List of parameter changes")
