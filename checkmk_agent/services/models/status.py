...
class ProblemSummary(BaseModel):
    ...
    urgent_problems: int = Field(description="Urgent problems requiring immediate attention")
    
    # Categorization
    problems_by_category: Dict[str, int] = Field(default_factory=dict, description="Problems by category")
    problems_by_host: Dict[str, int] = Field(default_factory=dict, description="Problems by host")
    
    # Recent trends
    new_problems_last_hour: int = Field(description="New problems in last hour")
    resolved_problems_last_hour: int = Field(description="Resolved problems in last hour")
...
class HealthDashboard(BaseModel):
    ...
    critical_problems: List[ServiceProblem] = Field(description="Critical service problems")
    urgent_problems: List[ServiceProblem] = Field(description="Urgent problems needing attention")
    
    # Host status
    host_statuses: List[HostStatus] = Field(description="Individual host statuses")
    worst_performing_hosts: List[HostStatus] = Field(description="Hosts with lowest health scores")
...
class StatusFilterOptions(BaseModel):
    ...
    service_filter: Optional[str] = Field(None, description="Service name pattern filter")
    state_filter: Optional[List[ServiceState]] = Field(None, description="Service states to include")
    severity_filter: Optional[List[ProblemSeverity]] = Field(None, description="Problem severities to include")
    category_filter: Optional[List[ProblemCategory]] = Field(None, description="Problem categories to include")
    
    # Time-based filters
    problems_since: Optional[datetime] = Field(None, description="Only show problems since this time")
    acknowledged_filter: Optional[bool] = Field(None, description="Filter by acknowledgement status")
    downtime_filter: Optional[bool] = Field(None, description="Filter by downtime status")
