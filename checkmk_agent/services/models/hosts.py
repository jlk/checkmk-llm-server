...
class HostListResult(BaseModel):
    ...
    # Summary statistics
    stats: Dict[str, int] = Field(default_factory=dict, description="Host state statistics")
...
class HostUpdateResult(BaseModel):
    ...
    message: str = Field(description="Success or error message")
    warnings: List[str] = Field(default_factory=list, description="Any warnings during update")
...
class HostBulkCreateResult(BaseModel):
    ...
    failed_hosts: List[Dict[str, str]] = Field(default_factory=list, description="Failed host creations with errors")
    ...
    total_requested: int = Field(description="Total number of hosts requested to create")
...
class HostBulkDeleteResult(BaseModel):
    ...
    failed_hosts: List[Dict[str, str]] = Field(default_factory=list, description="Failed host deletions with errors")
    ...
    total_requested: int = Field(description="Total number of hosts requested to delete")
    warnings: List[str] = Field(default_factory=list, description="General warnings")