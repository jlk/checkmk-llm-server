...
class TrendingParameterFilter(ParameterFilterStrategy):
    ...
    def should_include_parameter(
        ...
        if context.get("include_trending", False):
            logger.debug(f"Including trending parameter '{param_name}' due to explicit user request")
            return True
        ...
        if any(p in existing_params for p in self.TRENDING_PARAMS):
            logger.debug(f"Including trending parameter '{param_name}' to preserve existing configuration")
            return True
            
        # Default: exclude trending parameters
        logger.debug(f"Excluding trending parameter '{param_name}' (not explicitly requested)")
        return False
    ...
    def filter_parameters(
        ...
        for param_name, param_value in parameters.items():
            ...
            for filter_name, filter_strategy in self.filters.items():
                if not filter_strategy.should_include_parameter(param_name, param_value, context):
                    should_include = False
                    filter_reason = filter_strategy.get_filter_reason(param_name, param_value, context)
                    break
        ...
        if len(filtered) < len(parameters):
            ...
            self.logger.info(f"Filtered out {filtered_count} parameter(s) based on policies")
