...
class BaseParameterHandler(ABC):
    ...
    def apply_parameter_policies(
        ...
        """
        return self.parameter_policies.filter_parameters(parameters, context or {})
    ...
    def _create_validation_message(
        ...
        return ValidationMessage(
            severity=severity, message=message, field=field, suggestion=suggestion
        )
    ...
    def _validate_threshold_tuple(
        ...
        if (
            len(value) >= 2
            and field_name.endswith("levels")
            and not field_name.endswith("levels_lower")
        ):
            try:
    ...
    def _normalize_threshold_tuple(
        self, value: Any, numeric_type: type = float, length: int = 2
    ...
    def _validate_percentage(
        self, value: Any, field_name: str, allow_over_100: bool = False
    ) -> List[ValidationMessage]:
        """
    ...
    def _validate_positive_number(
        self, value: Any, field_name: str, numeric_type: type = float
    ) -> List[ValidationMessage]:
        """
    ...
    def _validate_choice(
        self, value: Any, field_name: str, choices: List[Any]
    ) -> List[ValidationMessage]:
        """
