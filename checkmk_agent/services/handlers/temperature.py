...
class TemperatureParameterHandler(BaseParameterHandler):
    ...
    def get_default_parameters(self, service_name: str, context: Optional[Dict[str, Any]] = None) -> HandlerResult:
        ...
        profile = self.TEMPERATURE_PROFILES.get(sensor_type, self.TEMPERATURE_PROFILES['generic'])
        ...
        if sensor_type == 'cpu':
            ...
            parameters['trend_compute']['period'] = 15
            parameters['device_levels_handling'] = 'average'  # Average for multi-core CPUs
        elif sensor_type == 'ambient':
            # Ambient temperature changes slowly
            parameters['trend_compute']['period'] = 60
            parameters['trend_compute']['trend_levels'] = (3.0, 5.0)  # Slower changes
        ...
        if context:
            ...
            if context.get('environment') == 'datacenter':
                ...
                parameters['levels'] = (warn - 5, crit - 5)
        ...
        messages = [
            ...
            self._create_validation_message(
                ValidationSeverity.INFO,
                f"Default thresholds: Warning {parameters['levels'][0]}°C, Critical {parameters['levels'][1]}°C"
            )
        ]
    ...
    def validate_parameters(
        ...
        unit_messages = self._validate_choice(output_unit, 'output_unit', ['c', 'f', 'k'])
        ...
        if 'levels' in parameters:
            ...
            if not threshold_messages:  # Only if basic validation passed
                ...
                warn_c, crit_c = self._convert_to_celsius(warn_temp, crit_temp, output_unit)
        ...
        if 'trend_compute' in parameters:
            trend_messages = self._validate_trend_parameters(parameters['trend_compute'])
    ...
    def suggest_parameters(
        ...
        # Suggest trend monitoring if not configured
        if 'trend_compute' not in current:
            suggestions.append({
    ...
    def _detect_sensor_type(self, service_name: str) -> str:
        ...
        if any(keyword in service_lower for keyword in ['cpu', 'processor', 'core']):
            ...
            return 'ambient'
        elif any(keyword in service_lower for keyword in ['disk', 'hdd', 'ssd', 'drive']):
            return 'disk'
        elif any(keyword in service_lower for keyword in ['psu', 'power', 'supply']):
            return 'psu'
        elif any(keyword in service_lower for keyword in ['motherboard', 'mainboard', 'system']):
            return 'motherboard'
        elif any(keyword in service_lower for keyword in ['case', 'chassis', 'enclosure']):
            return 'case'
    ...
    def _validate_trend_parameters(self, trend_config: Any) -> List:
        ...
        if 'trend_levels' in trend_config:
            trend_messages = self._validate_threshold_tuple(
                trend_config['trend_levels'],
                'trend_compute.trend_levels'
            )
            messages.extend(trend_messages)
        
        if 'trend_levels_lower' in trend_config:
            lower_trend_messages = self._validate_threshold_tuple(
                trend_config['trend_levels_lower'],
                'trend_compute.trend_levels_lower'
            )
            messages.extend(lower_trend_messages)
        
        return messages
    
    def _validate_threshold_consistency(self, upper_levels: tuple, lower_levels: tuple, unit: str) -> List:
        ...
        try:
            ...
            upper_warn_c, upper_crit_c = self._convert_to_celsius(upper_warn, upper_crit, unit)
            lower_warn_c, lower_crit_c = self._convert_to_celsius(lower_warn, lower_crit, unit)
