...
class CustomCheckParameterHandler(BaseParameterHandler):
    ...
    def get_default_parameters(
        ...
        profile = self.CUSTOM_CHECK_PROFILES.get(
            check_type, self.CUSTOM_CHECK_PROFILES["local"]
        )
        ...
        if check_type == "nagios_plugin":
            parameters.update(
                {"timeout": 30, "perfdata": True, "check_type": "nagios_plugin"}
            )
            ...
            parameters.update(
                {"check_interval": 60, "timeout": 60, "check_type": "mrpe"}
            )
        elif check_type == "script":
            parameters.update({"timeout": 60, "check_type": "script"})
        ...
        if context:
            ...
            if context.get("command_info"):
                ...
                if "slow" in str(cmd_info).lower() or "long" in str(cmd_info).lower():
                    ...
                    "fast" in str(cmd_info).lower() or "quick" in str(cmd_info).lower()
                ):
            ...
            if context.get("sample_perfdata"):
                ...
                parameters["perfdata_regex"] = self._generate_perfdata_regex(
                    context["sample_perfdata"]
                )
        ...
        messages = [
            ...
            self._create_validation_message(
                ValidationSeverity.INFO, f"Default check type: {check_type}"
            ),
        ]
    ...
    def validate_parameters(
        ...
        if "perfdata_regex" in parameters:
            regex_messages = self._validate_regex_parameter(
                parameters["perfdata_regex"], "perfdata_regex"
            )
        ...
        if "state_translation" in parameters or "state_mapping" in parameters:
            state_param = (
                "state_translation"
                if "state_translation" in parameters
                else "state_mapping"
            )
            state_messages = self._validate_state_mapping(
                parameters[state_param], state_param
            )
        ...
        for cmd_param in ["command_line", "command"]:
            if cmd_param in parameters:
                cmd_messages = self._validate_command_parameter(
                    parameters[cmd_param], cmd_param
                )
    ...
    def suggest_parameters(
        ...
        if check_type == "nagios_plugin" and (
            current_timeout is None or current_timeout > 60
        ):
            suggestions.append(
                {
                    "parameter": "timeout",
    ...
    def _detect_check_type(
        ...
        if "mrpe" in service_lower:
            ...
            pattern in service_lower for pattern in ["check_", "nagios", "plugin"]
        ):
    ...
    def _validate_mrpe_parameters(self, parameters: Dict[str, Any]) -> List:
        ...
        if "check_interval" in parameters:
            interval_messages = self._validate_positive_number(
                parameters["check_interval"], "check_interval", int
            )
