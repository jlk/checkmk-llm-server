...
class DatabaseParameterHandler(BaseParameterHandler):
    ...
    DATABASE_PROFILES = {
        'oracle': DatabaseProfile(
            ...
            common_metrics=['tablespaces', 'sessions', 'locks', 'archive_logs', 'instance', 'asm'],
            default_parameters={
                'tablespaces': {
            ...
            connection_parameters=['hostname', 'port', 'sid', 'service_name', 'username', 'password'],
            performance_parameters=['sga_target', 'pga_target', 'shared_pool_size', 'buffer_cache_size']
        ),
        'mysql': DatabaseProfile(
            db_type='mysql',
            description='MySQL/MariaDB monitoring',
            common_metrics=['connections', 'innodb', 'replication', 'slow_queries', 'table_locks'],
            default_parameters={
                'connections': {
            ...
            connection_parameters=['hostname', 'port', 'database', 'username', 'password', 'socket'],
            performance_parameters=['innodb_buffer_pool_size', 'query_cache_size', 'max_connections']
        ),
        'postgresql': DatabaseProfile(
            db_type='postgresql',
            description='PostgreSQL monitoring',  
            common_metrics=['connections', 'locks', 'bgwriter', 'replication', 'vacuum', 'databases'],
            default_parameters={
                'connections': {
                    'levels': (80.0, 90.0),  # % of max connections
                ...
                'replication': {
                    'lag_levels': (1024*1024, 10*1024*1024),  # Bytes behind primary
            ...
            connection_parameters=['hostname', 'port', 'database', 'username', 'password'],
            performance_parameters=['shared_buffers', 'work_mem', 'maintenance_work_mem', 'max_connections']
        ),
        'mongodb': DatabaseProfile(
            db_type='mongodb',
            description='MongoDB monitoring',
            common_metrics=['connections', 'replication', 'collections', 'indexes', 'operations'],
            default_parameters={
                'connections': {
                    'levels': (80.0, 90.0),  # % of max connections
            ...
            connection_parameters=['hostname', 'port', 'database', 'username', 'password', 'replica_set'],
            performance_parameters=['cache_size', 'connections', 'oplog_size']
        ),
        'mssql': DatabaseProfile(
            db_type='mssql',
            description='Microsoft SQL Server monitoring',
            common_metrics=['connections', 'databases', 'locks', 'memory', 'transactions', 'backup'],
            default_parameters={
                'connections': {
                    'levels': (80.0, 90.0),  # % of max connections
            ...
            connection_parameters=['hostname', 'port', 'database', 'username', 'password', 'instance'],
            performance_parameters=['max_memory', 'min_memory', 'max_degree_parallelism', 'cost_threshold']
        ),
        'redis': DatabaseProfile(
            db_type='redis',
            description='Redis monitoring',
            common_metrics=['memory', 'connections', 'persistence', 'replication', 'keyspace'],
            default_parameters={
                'memory': {
                ...
                'persistence': {
                    ...
                    'aof_size_levels': (100*1024*1024, 500*1024*1024)  # AOF file size
                },
                'replication': {
                    'lag_levels': (10, 30),  # Replication lag in seconds
        ...
        if not profile:
            ...
            if metric_type and metric_type in profile.default_parameters:
                ...
                if profile.connection_parameters:
                    parameters['connection_params'] = {
                        param: None for param in profile.connection_parameters[:3]  # First 3 most important
                    }
            ...
            if context:
                ...
                if context.get('environment') == 'production':
                    if 'levels' in parameters and isinstance(parameters['levels'], tuple):
                        warn, crit = parameters['levels']
                        parameters['levels'] = (warn - 5, crit - 5)  # Lower thresholds
                    
                    if 'connection_timeout' in parameters:
                        parameters['connection_timeout'] = min(parameters['connection_timeout'], 20)
            ...
            messages = [
                ...
                self._create_validation_message(
                    ValidationSeverity.INFO,
                    f"Metric type: {metric_type or 'general'}"
                )
            ]
    ...
    def validate_parameters(
        ...
        if profile:
            if db_type == 'oracle':
                oracle_messages = self._validate_oracle_parameters(parameters, metric_type)
                messages.extend(oracle_messages)
            elif db_type == 'mysql':
                mysql_messages = self._validate_mysql_parameters(parameters, metric_type)
                messages.extend(mysql_messages)
            elif db_type == 'postgresql':
                postgres_messages = self._validate_postgresql_parameters(parameters, metric_type)
                messages.extend(postgres_messages)
            elif db_type == 'mongodb':
                mongo_messages = self._validate_mongodb_parameters(parameters, metric_type)
                messages.extend(mongo_messages)
            elif db_type == 'mssql':
                mssql_messages = self._validate_mssql_parameters(parameters, metric_type)
    ...
    def _detect_database_and_metric(self, service_name: str, context: Optional[Dict[str, Any]] = None) -> tuple:
    ...
    def _validate_oracle_parameters(self, parameters: Dict[str, Any], metric_type: Optional[str]) -> List:
    ...
    def _validate_mysql_parameters(self, parameters: Dict[str, Any], metric_type: Optional[str]) -> List:
        ...
        if metric_type == 'innodb':
            ...
            if 'buffer_pool_hit_rate' in parameters:
                hit_rate_messages = self._validate_threshold_tuple(
                    parameters['buffer_pool_hit_rate'],
                    'buffer_pool_hit_rate'
                )
    ...
    def _validate_postgresql_parameters(self, parameters: Dict[str, Any], metric_type: Optional[str]) -> List:
    ...
    def _validate_mongodb_parameters(self, parameters: Dict[str, Any], metric_type: Optional[str]) -> List:
    ...
    def _validate_mssql_parameters(self, parameters: Dict[str, Any], metric_type: Optional[str]) -> List:
    ...
    def _validate_connection_params(self, conn_params: Any, expected_params: List[str]) -> List:
