...
class HandlerRegistry:
    ...
    def __init__(self):
        ...
        self._service_cache: Dict[str, List[str]] = {}  # Cache service -> handler mapping
        self._ruleset_cache: Dict[str, List[str]] = {}  # Cache ruleset -> handler mapping
    ...
    def register_handler(
        ...
        if handler_name in self._handlers:
            self.logger.warning(f"Handler '{handler_name}' is already registered, replacing")
        ...
        self.logger.debug(f"Registered handler '{handler_name}' with priority {priority}")
    ...
    def get_handlers_for_service(self, service_name: str, limit: int = 3) -> List[BaseParameterHandler]:
        """
        ...
        if cache_key in self._service_cache:
            ...
            return [self.get_handler(name) for name in handler_names if self.get_handler(name)]
        ...
        matching_handlers.sort(key=lambda x: x[1])  # Sort by priority (lower = higher priority)
        handlers = [handler for handler, _ in matching_handlers[:limit]]
        
        # Cache the result
        self._service_cache[cache_key] = [h.name for h in handlers]
        
        self.logger.debug(f"Found {len(handlers)} handlers for service '{service_name}': {[h.name for h in handlers]}")
        return handlers
    
    def get_handlers_for_ruleset(self, ruleset: str, limit: int = 3) -> List[BaseParameterHandler]:
        """
        ...
        self.logger.debug(f"Found {len(handlers)} handlers for ruleset '{ruleset}': {[h.name for h in handlers]}")
        return handlers
    ...
    def get_best_handler(
        ...
        if service_name and ruleset:
            ...
            service_handlers = set(h.name for h in self.get_handlers_for_service(service_name))
            ruleset_handlers = set(h.name for h in self.get_handlers_for_ruleset(ruleset))
            ...
            if common_handlers:
                ...
                best_handler_name = min(
                    common_handlers,
                    key=lambda name: self._handlers[name].priority
                )
...
def _register_default_handlers(registry: HandlerRegistry) -> None:
    ...
    try:
        ...
        logging.getLogger(__name__).warning(f"Could not register some default handlers: {e}")