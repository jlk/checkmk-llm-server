...
class NetworkServiceParameterHandler(BaseParameterHandler):
    ...
    def get_default_parameters(
        ...
        if not profile:
            ...
            parameters = {"response_time": (1.0, 3.0), "timeout": 10, "retry_count": 3}
            messages = [
    ...
    def validate_parameters(
        ...
        if "response_time" in parameters:
            response_messages = self._validate_threshold_tuple(
                parameters["response_time"], "response_time", numeric_type=float
            )
        ...
        if "timeout" in parameters:
            timeout_messages = self._validate_positive_number(
                parameters["timeout"], "timeout", int
            )
    ...
    def suggest_parameters(
        ...
        if "response_time" not in current:
            default_thresholds = profile.common_parameters.get(
        ...
        if "retry_count" not in current and any(
            keyword in service_name.lower()
            for keyword in ["prod", "critical", "important"]
        ):
            suggestions.append(
                {
        ...
        if (
            service_type == "http"
            and current.get("method") == "GET"
            and "health" in service_name.lower()
        ):
            suggestions.append(
                {
    ...
    def _validate_http_parameters(
        self, parameters: Dict[str, Any], service_type: str
    ) -> List:
        ...
        if "method" in parameters:
            ...
            method_messages = self._validate_choice(
                parameters["method"], "method", valid_methods
            )
        ...
        if service_type == "https":
            if "ssl_cert_age" in parameters:
                cert_messages = self._validate_threshold_tuple(
                    parameters["ssl_cert_age"], "ssl_cert_age", numeric_type=int
                )
            ...
            if "ssl_min_version" in parameters:
                ...
                ssl_version_messages = self._validate_choice(
                    parameters["ssl_min_version"], "ssl_min_version", valid_versions
                )
    ...
    def _validate_tcp_parameters(self, parameters: Dict[str, Any]) -> List:
        ...
        if "refuse_state" in parameters:
            ...
            refuse_messages = self._validate_choice(
                (
                    parameters["refuse_state"].lower()
                    if isinstance(parameters["refuse_state"], str)
                    else parameters["refuse_state"]
                ),
                "refuse_state",
                valid_states,
            )
    ...
    def _validate_dns_parameters(self, parameters: Dict[str, Any]) -> List:
        ...
        if "query_type" in parameters:
            ...
            type_messages = self._validate_choice(
                (
                    parameters["query_type"].upper()
                    if isinstance(parameters["query_type"], str)
                    else parameters["query_type"]
                ),
                "query_type",
                valid_types,
            )
    ...
    def _validate_url(self, url: str) -> List:
        ...
        if not isinstance(url, str) or not url.strip():
            messages.append(
                self._create_validation_message(
                    ValidationSeverity.ERROR, "URL must be a non-empty string", "url"
                )
            )
            return messages
        ...
        try:
            ...
            if not parsed.netloc:
                messages.append(
                    self._create_validation_message(
                        ValidationSeverity.ERROR, "URL must include a hostname", "url"
                    )
                )
            ...
            messages.append(
                self._create_validation_message(
                    ValidationSeverity.ERROR, f"Invalid URL format: {e}", "url"
                )
            )
    ...
    def _parse_url(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            parsed = urlparse(url)
            result = {"url": url, "hostname": parsed.hostname, "scheme": parsed.scheme}
