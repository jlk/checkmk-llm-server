    ...
    def decorator(func: F) -> F:
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                await get_metrics_collector().record_timing(name, duration, **(tags or {}))
                await get_metrics_collector().increment_counter(f"{name}.success")
                return result
                
            except Exception as e:
                duration = time.time() - start_time
                await get_metrics_collector().record_timing(f"{name}.error", duration, **(tags or {}))
...
async def timed_context(metric_name: str, tags: Optional[Dict[str, str]] = None):
    ...
    try:
        yield
        duration = time.time() - start_time
        await get_metrics_collector().record_timing(metric_name, duration, **(tags or {}))
        await get_metrics_collector().increment_counter(f"{metric_name}.success")
        
    except Exception as e:
        duration = time.time() - start_time
        await get_metrics_collector().record_timing(f"{metric_name}.error", duration, **(tags or {}))
...
class MetricsMixin:
    ...
    async def record_operation(self, operation: str, duration: float, success: bool = True, **tags):
        """Record an operation metric."""
        await self.metrics.record_timing(f"{self.__class__.__name__}.{operation}", duration, **tags)
        
        if success:
            await self.metrics.increment_counter(f"{self.__class__.__name__}.{operation}.success", **tags)
        else:
            await self.metrics.increment_counter(f"{self.__class__.__name__}.{operation}.error", **tags)
    
    async def record_api_call(self, endpoint: str, duration: float, status_code: int = 200):
    ...
    async def get_service_metrics(self) -> Dict[str, Any]:
        ...
        service_timing = {
            k: v for k, v in all_stats['timing_stats'].items()
            if k.startswith(service_name)
        }
        
        service_counters = {
            k: v for k, v in all_stats['counters'].items()
            if k.startswith(service_name)
        }
        ...
        return {
            ...
            'avg_response_time': statistics.mean([
                v['avg_ms'] for v in service_timing.values()
            ]) if service_timing else 0
        }