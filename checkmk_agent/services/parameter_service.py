...
class ParameterService(BaseService):
    ...
    DEFAULT_PARAMETERS = {
        ...
        "cpu_load": {"levels": (5.0, 10.0)},  # Load average thresholds
        ...
        "mysql_connections": {"levels": (80.0, 90.0)},  # % of max connections
        # Custom checks
        "custom_checks": {
        ...
        # Network services
        "tcp_connections": {"levels": (80, 100)},  # Number of connections
        "http": {
            "response_time": (1.0, 2.0),  # Response time in seconds
            "timeout": 10.0,
        },
        "ssl_certificates": {"age": (30, 7)},  # Days before expiry
    }
    ...
    async def get_default_parameters(
        self, service_type: str, context: Optional[Dict[str, Any]] = None
        ...
        async def _get_defaults_operation():
            ...
            handlers = self.handler_registry.get_handlers_for_service(
                service_type, limit=1
            )
            if handlers:
                handler = handlers[0]
                try:
                    handler_result = handler.get_default_parameters(
                    ...
                    if handler_result.success:
                        return {
                            ...
                            "handler_messages": [
                                msg.message
                                for msg in handler_result.validation_messages
                            ],
                            "message": f"Using specialized {handler.name} handler for {service_type}",
                        }
                except Exception as e:
                    self.logger.warning(
                        f"Handler {handler.name} failed for service {service_type}: {e}"
                    )
        ...
        return await self._execute_with_error_handling(
            _get_defaults_operation, f"get_default_parameters_{service_type}"
        )
    ...
    async def get_effective_parameters(
        ...
        async def _get_effective_operation():
            ...
            try:
                # Get effective parameters from Checkmk API using correct implementation
                effective_result = await self.checkmk.get_service_effective_parameters(
                    host_name, service_name
                )

                # Check if the API call was successful
                if effective_result.get("status") == "error":
                    # Handle API errors by falling back to default parameters
                    default_params = self.DEFAULT_PARAMETERS.get(service_type, {})
                    return ServiceParameterResult(
                        host_name=host_name,
                        service_name=service_name,
                        success=True,
                        message=f"Using default parameters (API error: {effective_result.get('parameters', {}).get('error', 'Unknown error')})",
                        parameters=default_params,
                        effective_parameters=default_params,
                        ruleset=ruleset,
                        warnings=[
                            f"Could not retrieve effective parameters: {effective_result.get('parameters', {}).get('error', 'Unknown error')}"
                        ],
                    )

                # Extract the parameters from the improved API response
                effective_params = effective_result.get("parameters", {})
                matching_rules = effective_result.get("matching_rules", [])
                used_ruleset = effective_result.get("ruleset", ruleset)

                # Build warnings for better user feedback
                warnings = []
                if effective_result.get("rule_count", 0) == 0:
                    warnings.append(
                        "No matching rules found - service may use default check parameters"
                    )
                elif used_ruleset == "unknown":
                    warnings.append(
                        f"Could not determine specific ruleset for service '{service_name}'"
                    )
                ...
                return ServiceParameterResult(
                    ...
                    service_name=service_name,
                    success=True,
                    message=f"Retrieved effective parameters for {host_name}/{service_name} (found {effective_result.get('rule_count', 0)} matching rules)",
                    parameters=effective_params,
                    effective_parameters=effective_result,  # Include full result for advanced usage
        ...
        return await self._execute_with_error_handling(
            _get_effective_operation,
            f"get_effective_parameters_{host_name}_{service_name}",
        )
    ...
    async def set_service_parameters(
        ...
        """
        Set custom parameters for a specific service.
        Implements update-before-create logic: looks for existing rules first,
        updates if found, otherwise creates a new rule.
        ...
        Args:
            ...
            rule_properties: Optional rule properties
            context: Optional context including user intent
        ...
        async def _set_parameters_operation():
            ...
            if not ruleset:
                ...
                if discovery_result.success and discovery_result.data:
                    ruleset = discovery_result.data.get("recommended_ruleset")
                    if not ruleset:
                        discovered = discovery_result.data.get(
                            "discovered_rulesets", []
                        )
                        raise ValueError(
            ...
            # STEP 1: Check for existing rule first (UPDATE-BEFORE-CREATE logic)
            existing_rule = await self._find_existing_rule_for_service(
                host_name, service_name, ruleset
            )
            ...
            # Apply parameter policies using handler if available, otherwise use global policies
            ...
            if validation_result.success:
                ...
                # If validation service fails, continue with filtered parameters
                self.logger.warning(
                    f"Could not validate parameters: {validation_result.error}"
                )
                validated_params = filtered_parameters

            rule_id = None
            was_updated = False

            if existing_rule:
                # STEP 2: Replace existing rule (delete + create with updated parameters)
                self.logger.info(
                    f"Found existing rule {existing_rule.get('id')} for {host_name}/{service_name}, replacing with updated parameters. "
                    f"Rule details: folder='{existing_rule.get('folder')}', "
                    f"ruleset='{existing_rule.get('ruleset')}', "
                    f"description='{existing_rule.get('properties', {}).get('description', 'No description')}', "
                    f"conditions={existing_rule.get('conditions', {})}"
                )

                try:
                    # Replace the existing rule with new parameters using the replace strategy
                    replace_result = await self.update_parameter_rule(
                        rule_id=existing_rule["id"],
                        parameters=validated_params,
                        preserve_conditions=True,  # Keep existing host/service conditions
                        rule_properties=rule_properties,
                    )

                    if replace_result.success:
                        replace_data = replace_result.data
                        rule_id = replace_data.get(
                            "new_rule_id"
                        )  # Use new rule ID from replace operation
                        was_updated = True
                        self.logger.info(
                            f"Successfully replaced existing rule {existing_rule['id']} with new rule {rule_id}"
                        )
                    else:
                        # If replace fails, log warning and fall back to create
                        self.logger.warning(
                            f"Failed to replace existing rule {existing_rule['id']}: {replace_result.error}. "
                            f"Falling back to creating new rule."
                        )
                        existing_rule = None  # Clear to trigger create path

                except Exception as e:
                    # If replace fails, log warning and fall back to create
                    self.logger.warning(
                        f"Exception replacing existing rule {existing_rule['id']}: {e}. "
                        f"Falling back to creating new rule."
                    )
                    existing_rule = None  # Clear to trigger create path

            # STEP 3: Create new rule if no existing rule found or update failed
            if not existing_rule:
                self.logger.info(
                    f"No existing rule found for {host_name}/{service_name}, creating new rule"
                )
                ...
                if "comment" not in properties:
                    properties["comment"] = (
                        rule_comment
                        or f"Parameter override for {host_name}/{service_name}"
                    )
                ...
                # Create the rule using the API
                rule_result = await self.checkmk.create_service_parameter_rule(
                    ...
                    description=properties.get(
                        "comment", f"Parameter override for {host_name}/{service_name}"
                    ),
                )
                rule_id = rule_result.get("id")
                was_updated = False

            # Get effective parameters after rule creation/update
            try:
                effective_params = await self.checkmk.get_service_effective_parameters(
                    host_name, service_name
                )
            except Exception as e:
                self.logger.warning(
                    f"Could not retrieve effective parameters after rule {'update' if was_updated else 'creation'}: {e}"
                )
                effective_params = {"parameters": validated_params, "status": "error"}

            changes_made = []
            for key, value in validated_params.items():
                changes_made.append(f"{key} = {value}")

            # Build result message based on whether we updated or created
            if was_updated:
                message = f"Successfully replaced existing parameter rule for {host_name}/{service_name}"
            else:
                message = f"Successfully created new parameter rule for {host_name}/{service_name}"

            # Add filtering information to message if parameters were filtered
            if len(validated_params) < len(parameters):
                filtered_count = len(parameters) - len(validated_params)
                message += f" ({filtered_count} parameter(s) filtered by policies)"

            # Automatically activate changes if configured to do so
            activation_warnings = []
            if self.config.checkmk.auto_activate_changes:
                try:
                    self.logger.info(
                        f"Auto-activating changes after parameter rule {'update' if was_updated else 'creation'} for {host_name}/{service_name}"
                    )
                    activation_result = await self.checkmk.activate_changes()

                    if activation_result.get("status") == "no_changes":
                        self.logger.debug("No pending changes to activate")
                    elif activation_result.get("status") == "already_running":
                        self.logger.warning(
                            "Activation already in progress, changes will be applied automatically"
                        )
                        activation_warnings.append(
                            "Configuration activation already in progress"
                        )
                    else:
                        self.logger.info(
                            "Successfully activated parameter rule changes"
                        )

                except Exception as e:
                    # Log activation failures but don't fail the main operation
                    self.logger.warning(
                        f"Failed to automatically activate changes after parameter rule {'update' if was_updated else 'creation'}: {e}"
                    )
                    activation_warnings.append(
                        f"Failed to activate changes automatically: {e}"
                    )
            ...
            if was_updated:
                warnings.append(
                    f"Replaced existing rule using replace strategy, new rule ID: {rule_id}"
                )
            warnings.extend(activation_warnings)
            warnings.extend(filter_messages)  # Add parameter filtering messages
        ...
        return await self._execute_with_error_handling(
            _set_parameters_operation, f"set_parameters_{host_name}_{service_name}"
        )

    async def discover_ruleset_dynamic(
        ...
        async def _dynamic_discovery_operation():
            ...
            try:
                ...
                for ruleset_info in all_rulesets:
                    ...
                    if service_type in ruleset_name:
                        ...
                        match_reasons.append(
                            f"service type '{service_type}' in ruleset name"
                        )
                        ...
                        results["discovered_rulesets"].append(
                            {
                                ...
                                "confidence": (
                                    "high"
                                    if match_score >= 10
                                    else "medium" if match_score >= 5 else "low"
                                ),
                            }
                        )
                ...
                if results["discovered_rulesets"]:
                    ...
                    if (
                        not results["recommended_ruleset"]
                        or best_match.get("match_score", 0) > 5
                    ):
                ...
                if not results["recommended_ruleset"]:
                    check_patterns = [r"^Check_MK\s+(\w+)", r"^(\w+)\s+", r"^(\w+)$"]
                    import re
                    ...
                    for pattern in check_patterns:
                        ...
                        if match:
                            ...
                            for ruleset_info in all_rulesets:
                                ruleset_id = ruleset_info.get("id", "")
                                if (
                                    f"checkgroup_parameters:{potential_plugin}"
                                    == ruleset_id
                                ):
                                    results["discovered_rulesets"].append(
                                        {
        ...
        return await self._execute_with_error_handling(
            _dynamic_discovery_operation, f"discover_ruleset_dynamic_{service_name}"
        )

    async def discover_ruleset(
        self, service_name: str
    ) -> ServiceResult[Dict[str, Any]]:
        """
        ...
        async def _discover_ruleset_operation():
            ...
            try:
                ...
                for ruleset_info in available_rulesets:
                    ...
                    if (
                        service_type in ruleset_name
                        or service_type in ruleset_title
                        or any(
                            keyword in ruleset_name for keyword in service_lower.split()
                        )
                    ):
        ...
        return await self._execute_with_error_handling(
            _discover_ruleset_operation, f"discover_ruleset_{service_name}"
        )
    ...
    async def list_parameter_rules(
        ...
        async def _list_rules_operation():
            ...
            if ruleset:
                ...
                try:
                    ...
                    all_rulesets = (
                        all_rulesets_response.get("value", [])
                        if isinstance(all_rulesets_response, dict)
                        else all_rulesets_response
                    )
                    ...
                    for ruleset_name in parameter_rulesets:
                        try:
                            ...
                            self.logger.warning(
                                f"Could not retrieve rules for ruleset {ruleset_name}: {e}"
                            )
        ...
        return await self._execute_with_error_handling(
            _list_rules_operation, "list_parameter_rules"
        )
    ...
    async def get_parameter_schema(
        ...
        async def _get_schema_operation():
            try:
                ...
                if valuespec:
                    schema_data["parameter_definitions"] = self._parse_valuespec(
                        valuespec
                    )
                ...
                self.logger.warning(
                    f"Could not retrieve schema for ruleset {ruleset_name}: {e}"
                )
        ...
        return await self._execute_with_error_handling(
            _get_schema_operation, f"get_parameter_schema_{ruleset_name}"
        )
    ...
    async def validate_parameters(
        ...
        async def _validate_operation():
            ...
            if service_name:
                ...
                if not handlers:
                    handlers = self.handler_registry.get_handlers_for_ruleset(
                        ruleset, limit=1
                    )
                ...
                if handlers:
                    handler = handlers[0]
                    try:
                        handler_result = handler.validate_parameters(
                            parameters, service_name or ruleset, context
                        )
                        if handler_result.success:
                            # Convert handler result to ValidationResult format
                            handler_errors = [
                                msg.message for msg in handler_result.errors
                            ]
                            handler_warnings = [
                                msg.message for msg in handler_result.warnings
                            ]
                            ...
                            warnings.append(
                                f"Handler {handler.name} validation failed: {handler_result.error}"
                            )
                            handler_used = handler.name
                    except Exception as e:
                        self.logger.warning(
                            f"Handler {handler.name} validation error: {e}"
                        )
            ...
            if not schema_result.success:
                ...
                if handler_used:
                    basic_validation.warnings.append(
                        f"Used fallback validation (handler: {handler_used})"
                    )
                ...
                for param_name, param_def in param_defs.items():
                    if param_name == "_value":
                        ...
                        if param_name in parameters:
                            ...
                            warnings.extend(validation.get("warnings", []))
                            if validation.get("normalized_value") is not None:
                                normalized_params[param_name] = validation[
                                    "normalized_value"
                                ]

            # Special validation for known parameter types
            if "temperature" in ruleset:
                temp_validation = self._validate_temperature_parameters(
                    normalized_params
                )
                ...
                if "_value" not in known_params:
                    ...
                    for param in parameters:
                        if param not in known_params:
                            warnings.append(
                                f"Unknown parameter '{param}' for ruleset {ruleset}"
                            )
        ...
        return await self._execute_with_error_handling(
            _validate_operation, f"validate_parameters_{ruleset}"
        )

    def _validate_parameter_value(
        self, value: Any, param_def: Dict[str, Any], param_name: str
    ) -> Dict[str, Any]:
        ...
        if param_type == "tuple":
            ...
            if not isinstance(value, (list, tuple)):
                errors.append(
                    f"Parameter '{param_name}' must be a tuple/list, got {type(value).__name__}"
                )
            else:
                elements = param_def.get("elements", [])
                if elements and len(value) != len(elements):
                    errors.append(
                        f"Parameter '{param_name}' expects {len(elements)} values, got {len(value)}"
                    )
                else:
                    ...
                    for i, (val, expected_type) in enumerate(zip(value, elements)):
                        if expected_type in ["float", "percentage"]:
                            try:
                                ...
                                errors.append(
                                    f"Element {i} of '{param_name}' must be a number"
                                )
                            ...
                            try:
                                ...
                                errors.append(
                                    f"Element {i} of '{param_name}' must be an integer"
                                )
                        else:
            ...
            try:
                ...
                if "minvalue" in param_def and normalized_value < param_def["minvalue"]:
                    errors.append(
                        f"Parameter '{param_name}' must be >= {param_def['minvalue']}"
                    )
                if "maxvalue" in param_def and normalized_value > param_def["maxvalue"]:
                    errors.append(
                        f"Parameter '{param_name}' must be <= {param_def['maxvalue']}"
                    )

            except (TypeError, ValueError):
                errors.append(
                    f"Parameter '{param_name}' must be a {'n integer' if param_type == 'integer' else ' number'}"
                )
    ...
    def _validate_temperature_parameters(
        ...
        if "levels_lower" in params:
            ...
            if warn <= crit:
                errors.append(
                    "Lower temperature warning threshold must be greater than critical threshold"
                )
            if crit < -50 and params.get("output_unit", "c") == "c":
                warnings.append(
                    f"Critical lower temperature {crit}°C seems unusually low"
                )
    ...
    def _validate_filesystem_parameters(
        self, params: Dict[str, Any]
    ) -> Dict[str, List[str]]:
        ...
        if "levels" in params:
            ...
            if warn >= crit:
                errors.append(
                    "Filesystem warning threshold must be less than critical threshold"
                )
    ...
    def _basic_parameter_validation(
        self, ruleset: str, parameters: Dict[str, Any]
    ) -> ValidationResult:
        ...
        if "levels" in parameters:
            if (
                not isinstance(parameters["levels"], (list, tuple))
                or len(parameters["levels"]) != 2
            ):
                errors.append(
                    "'levels' must be a tuple of (warning, critical) thresholds"
                )
            else:
                try:
                    warn, crit = float(parameters["levels"][0]), float(
                        parameters["levels"][1]
                    )
                    if warn >= crit:
                        errors.append(
        ...
        warnings.append(
            f"Could not retrieve schema for {ruleset}, using basic validation only"
        )
    ...
    def _filter_trending_parameters(
        ...
        """
        self.logger.warning(
            "Using deprecated _filter_trending_parameters method. "
            "Consider using policy-based filtering instead."
        )
        # Simple fallback - just return parameters unchanged
        return parameters
    ...
    def _determine_service_type(self, service_name: str) -> str:
        ...
        if any(keyword in service_lower for keyword in ["cpu", "processor", "load"]):
            ...
            for keyword in ["disk", "filesystem", "df", "mount", "fs"]
        ):
            return "disk"
        elif any(
            keyword in service_lower
            for keyword in ["network", "interface", "eth", "bond"]
        ):
            return "network"
        elif any(
            keyword in service_lower
            for keyword in ["ping", "ssh", "tcp", "http", "connect"]
        ):
            return "network"
        else:
            return "unknown"
    ...
    def _rule_matches_filters(
        ...
        if host_name:
            ...
            if rule_hosts and host_name not in rule_hosts:
                ...
                if not any(
                    self._pattern_matches(pattern, host_name) for pattern in rule_hosts
                ):
                    return False
        ...
        if service_name:
            ...
            if rule_services and service_name not in rule_services:
                # Check for pattern matches
                if not any(
                    self._pattern_matches(pattern, service_name)
                    for pattern in rule_services
                ):
                    return False
    ...
    async def update_parameter_rule(
        ...
        """
        Update an existing parameter rule using a replace strategy.

        This method implements a replace strategy instead of direct update:
        1. Find and extract all properties from the existing rule
        2. Modify only the parameters while preserving everything else
        3. Delete the original rule
        4. Create a new rule with updated parameters and preserved properties

        This approach works around limitations in the Checkmk API's rule update functionality.
        ...
        async def _replace_rule_operation():
            # Validate parameters
            validation_errors = self._validate_required_params(
            ...
            self.logger.info(f"Starting replace operation for rule {rule_id}")

            # STEP 1: Get existing rule to extract all properties
            try:
                existing_rule = await self.checkmk.get_rule(rule_id)
                self.logger.debug(f"Retrieved existing rule {rule_id}: {existing_rule}")
            except CheckmkAPIError as e:
                if "not found" in str(e).lower():
                    raise ValueError(f"Rule {rule_id} not found")
                else:
                    raise ValueError(f"Could not retrieve existing rule {rule_id}: {e}")
            ...
            # STEP 2: Extract all rule properties that need to be preserved
            extensions = existing_rule.get("extensions", {})
            ruleset = extensions.get("ruleset")
            folder = extensions.get("folder", "/")
            original_conditions = extensions.get("conditions", {})
            original_properties = extensions.get("properties", {})
            original_parameters = extensions.get("value_raw", {})

            if not ruleset:
                raise ValueError(f"Could not determine ruleset for rule {rule_id}")

            self.logger.info(
                f"Extracted rule metadata - Ruleset: {ruleset}, Folder: {folder}, "
                f"Conditions: {original_conditions}, Properties: {original_properties}"
            )

            # STEP 3: Validate new parameters against ruleset schema if possible
            try:
                ...
                self.logger.warning(
                    f"Parameter validation failed: {e}, using original parameters"
                )
                validated_params = parameters

            # STEP 4: Prepare properties for the new rule
            new_properties = original_properties.copy()
            if rule_properties:
                new_properties.update(rule_properties)

            # Add timestamp to description if we're updating it
            if "description" in new_properties or "comment" in new_properties:
                description = new_properties.get("description") or new_properties.get(
                    "comment"
                )
                # Don't add timestamp if it's already timestamped to avoid duplicate timestamps
                if description and not any(
                    marker in description for marker in ["Updated:", "Created:", "[20"]
                ):
                    new_properties["description"] = f"Updated: {description}"

            # STEP 5: Prepare conditions for the new rule
            new_conditions = original_conditions.copy() if preserve_conditions else {}

            # Extract host and service patterns from conditions for create_service_parameter_rule
            host_name = None
            service_pattern = None

            if new_conditions:
                # Extract host_name from conditions
                host_condition = new_conditions.get("host_name", {})
                if isinstance(host_condition, dict) and "match_on" in host_condition:
                    match_on = host_condition["match_on"]
                    if isinstance(match_on, list) and len(match_on) > 0:
                        host_name = match_on[0]  # Use first host in the list

                # Extract service_pattern from conditions
                service_condition = new_conditions.get("service_description", {})
                if (
                    isinstance(service_condition, dict)
                    and "match_on" in service_condition
                ):
                    match_on = service_condition["match_on"]
                    if isinstance(match_on, list) and len(match_on) > 0:
                        service_pattern = match_on[0]  # Use first service pattern

            self.logger.info(
                f"Prepared new rule data - Host: {host_name}, Service: {service_pattern}, "
                f"Conditions preserved: {preserve_conditions}"
            )

            # STEP 6: Delete the original rule (CRITICAL: This is the point of no return)
            try:
                await self.checkmk.delete_rule(rule_id)
                self.logger.info(f"Successfully deleted original rule {rule_id}")
            except CheckmkAPIError as e:
                raise ValueError(
                    f"Failed to delete original rule {rule_id}: {e}. Operation aborted."
                )

            # STEP 7: Create new rule with updated parameters and preserved properties
            try:
                new_rule = await self.checkmk.create_service_parameter_rule(
                    ruleset_name=ruleset,
                    folder=folder,
                    parameters=validated_params,
                    host_name=host_name,
                    service_pattern=service_pattern,
                    description=new_properties.get(
                        "description", f"Replaced rule (original ID: {rule_id})"
                    ),
                )

                new_rule_id = new_rule.get("id")
                self.logger.info(
                    f"Successfully created new rule {new_rule_id} to replace {rule_id}"
                )
                ...
                return {
                    ...
                    "preserved_properties": new_properties,
                    "message": f"Successfully replaced rule {rule_id} with new rule {new_rule_id}",
                    "ruleset": ruleset,
                    "folder": folder,
                    "host_name": host_name,
                    "service_pattern": service_pattern,
            ...
            except CheckmkAPIError as create_error:
                # CRITICAL ERROR: We deleted the original rule but couldn't create the new one
                error_msg = (
                    f"CRITICAL: Original rule {rule_id} was successfully deleted, but creating "
                    f"the replacement rule failed: {create_error}. "
                    f"Original rule data: Ruleset={ruleset}, Folder={folder}, "
                    f"Host={host_name}, Service={service_pattern}, "
                    f"Parameters={original_parameters}, Properties={original_properties}"
                )
                self.logger.error(error_msg)
                raise ValueError(error_msg)

        return await self._execute_with_error_handling(
            _replace_rule_operation, f"replace_parameter_rule_{rule_id}"
        )

    async def find_parameter_rules(
        self, search_filter: RuleSearchFilter
    ) -> ServiceResult[List[Dict[str, Any]]]:
        """
        ...
        async def _find_rules_operation():
            ...
            if search_filter.rulesets:
                for ruleset in search_filter.rulesets:
                    try:
                        ruleset_rules = await self.checkmk.list_rules(
                            ruleset_name=ruleset
                        )
                        all_rules.extend(ruleset_rules)
                    except CheckmkAPIError as e:
                        self.logger.warning(
                            f"Could not retrieve rules for ruleset {ruleset}: {e}"
                        )
            else:
                ...
                try:
                    ...
                    for ruleset_name in common_parameter_rulesets:
                        try:
                            ...
                            self.logger.debug(
                                f"Ruleset {ruleset_name} not available: {ruleset_error}"
                            )
        ...
        return await self._execute_with_error_handling(
            _find_rules_operation, "find_parameter_rules"
        )
    ...
    async def set_bulk_parameters(
        ...
        async def _bulk_operations():
            if validate_all:
                ...
                for i, op in enumerate(operations):
                    ...
                    missing_fields = [
                        field for field in required_fields if field not in op
                    ]
                    if missing_fields:
                        validation_errors.append(
                            f"Operation {i}: Missing fields: {missing_fields}"
                        )
                        continue
                    ...
                    try:
                        ...
                        if not ruleset:
                            ...
                            discovery_result = await self.discover_ruleset_dynamic(
                                op["service_name"]
                            )
                            if discovery_result.success and discovery_result.data:
                                ruleset = discovery_result.data.get(
                                    "recommended_ruleset"
                                )

                        if ruleset:
                            validation_result = await self.validate_parameters(
                                ruleset, op["parameters"]
                            )
                            if (
                                validation_result.success
                                and not validation_result.data.is_valid
                            ):
                                validation_errors.append(
                        ...
                        validation_errors.append(
                            f"Operation {i}: Validation error: {str(e)}"
                        )

                if validation_errors:
                    raise ValueError(
                        f"Bulk validation failed: {'; '.join(validation_errors)}"
                    )
        ...
        return await self._execute_with_error_handling(
            _bulk_operations, "set_bulk_parameters"
        )

    def _rule_matches_search_filter(
        self, rule: Dict[str, Any], search_filter: RuleSearchFilter
    ) -> bool:
        ...
        if search_filter.host_patterns:
            ...
            # FIXED: Empty host conditions mean the rule applies to ALL hosts
            if rule_hosts:
                # Rule has specific host conditions - check if any match
                host_match = False
                for host_pattern in search_filter.host_patterns:
                    for rule_host in rule_hosts:
                        if self._pattern_matches(
                            rule_host, host_pattern
                        ) or self._pattern_matches(host_pattern, rule_host):
                            host_match = True
                            break
                    if host_match:
                        break
                ...
                if not host_match:
                    return False
            # If rule_hosts is empty, rule applies to all hosts - no need to check

        # Check service patterns
        if search_filter.service_patterns:
            rule_services = conditions.get("service_description", [])

            # FIXED: Empty service conditions mean the rule applies to ALL services
            if rule_services:
                # Rule has specific service conditions - check if any match
                service_match = False
                for service_pattern in search_filter.service_patterns:
                    for rule_service in rule_services:
                        if self._pattern_matches(
                            rule_service, service_pattern
                        ) or self._pattern_matches(service_pattern, rule_service):
                            service_match = True
                            break
                    if service_match:
                        break
                ...
                if not service_match:
                    return False
            # If rule_services is empty, rule applies to all services - no need to check
            ...
            for param_key, expected_value in search_filter.parameter_filters.items():
                ...
                if isinstance(expected_value, dict) and "operator" in expected_value:
                    ...
                    if op == "equals" and rule_value != value:
                        ...
                        return False
                    elif op == "greater_than" and (
                        not isinstance(rule_value, (int, float)) or rule_value <= value
                    ):
                        return False
                    elif op == "less_than" and (
                        not isinstance(rule_value, (int, float)) or rule_value >= value
                    ):
                        return False
                else:
    ...
    async def _find_existing_rule_for_service(
        self, host_name: str, service_name: str, ruleset: str
    ) -> Optional[Dict[str, Any]]:
        """
        Find existing parameter rule that matches the given host and service.

        Args:
            host_name: Target host name
            service_name: Target service name
            ruleset: Ruleset to search in

        Returns:
            First matching rule or None if no match found
        """
        try:
            self.logger.debug(
                f"RULE SEARCH DEBUG: Looking for existing rule - Host: '{host_name}', Service: '{service_name}', Ruleset: '{ruleset}'"
            )

            # Create search filter to find rules for this host/service combination
            search_filter = RuleSearchFilter(
                host_patterns=[host_name],
            ...
            self.logger.debug(
                f"RULE SEARCH DEBUG: Created search filter - Host patterns: {search_filter.host_patterns}, Service patterns: {search_filter.service_patterns}"
            )

            # Search for matching rules
            search_result = await self.find_parameter_rules(search_filter)
            if not search_result.success:
                self.logger.warning(
                    f"Could not search for existing rules: {search_result.error}"
                )
                return None

            matching_rules = search_result.data
            self.logger.debug(
                f"RULE SEARCH DEBUG: find_parameter_rules returned {len(matching_rules) if matching_rules else 0} rules"
            )

            if not matching_rules:
                self.logger.debug(
                    f"RULE SEARCH DEBUG: No matching rules found by find_parameter_rules"
                )
                return None

            # Log details of each rule returned by the search
            for i, rule in enumerate(matching_rules):
                ...
                rule_services = conditions.get("service_description", [])
                self.logger.debug(
                    f"RULE SEARCH DEBUG: Rule #{i+1}: ID={rule_id}, Folder='{folder}', Hosts={rule_hosts}, Services={rule_services}"
                )

            # Return the first matching rule (highest priority)
            # Rules are typically returned in priority order
            for i, rule in enumerate(matching_rules):
                # Double-check that this rule actually matches our criteria
                matches = self._rule_matches_host_service(
                    rule, host_name, service_name, ruleset
                )
                self.logger.debug(
                    f"RULE SEARCH DEBUG: Rule #{i+1} detailed match check: {matches}"
                )

                if not matches:
                    # Log why it doesn't match
                    self.logger.debug(
                        f"RULE SEARCH DEBUG: Rule #{i+1} failed detailed match check:"
                    )

                    # Check each condition
                    rule_ruleset = rule.get("ruleset")
                    self.logger.debug(
                        f"  - Ruleset match: '{rule_ruleset}' == '{ruleset}' -> {rule_ruleset == ruleset}"
                    )

                    properties = rule.get("properties", {})
                    disabled = properties.get("disabled", False)
                    self.logger.debug(f"  - Enabled: not {disabled} -> {not disabled}")

                    conditions = rule.get("conditions", {})
                    rule_hosts = conditions.get("host_name", [])
                    if rule_hosts:
                        host_matches = [
                            self._pattern_matches(pattern, host_name)
                            or pattern == host_name
                            for pattern in rule_hosts
                        ]
                        self.logger.debug(
                            f"  - Host match: patterns={rule_hosts}, matches={host_matches} -> any={any(host_matches)}"
                        )
                    else:
                        self.logger.debug(
                            f"  - Host match: No host restrictions (should match all)"
                        )
                    ...
                    if rule_services:
                        service_matches = [
                            self._pattern_matches(pattern, service_name)
                            or pattern == service_name
                            for pattern in rule_services
                        ]
                        self.logger.debug(
                            f"  - Service match: patterns={rule_services}, matches={service_matches} -> any={any(service_matches)}"
                        )
                    else:
                        self.logger.debug(
                            f"  - Service match: No service restrictions (should match all)"
                        )
                else:
                    self.logger.debug(
                        f"RULE SEARCH DEBUG: Found matching rule: ID={rule.get('id', 'N/A')}, Folder='{rule.get('folder', 'N/A')}'"
                    )
                    return rule

            self.logger.debug(
                f"RULE SEARCH DEBUG: No rules passed detailed match check"
            )
            return None

        except Exception as e:
            self.logger.error(
                f"Error finding existing rule for {host_name}/{service_name}: {e}"
            )
            return None
    ...
    def _rule_matches_host_service(
        ...
        """
        Check if a rule exactly matches the given host, service, and ruleset.

        Args:
            rule: Rule to check
            host_name: Target host name
            service_name: Target service name
            expected_ruleset: Expected ruleset name

        Returns:
            True if rule matches all criteria
        """
        # Check ruleset match
        if rule.get("ruleset") != expected_ruleset:
            return False
        ...
        rule_hosts = conditions.get("host_name", [])
        if rule_hosts:
            # Check if host matches any of the rule's host patterns
            host_matches = any(
                self._pattern_matches(pattern, host_name) or pattern == host_name
                for pattern in rule_hosts
            )
            if not host_matches:
                return False
        ...
        rule_services = conditions.get("service_description", [])
        if rule_services:
            # Check if service matches any of the rule's service patterns
            service_matches = any(
                self._pattern_matches(pattern, service_name) or pattern == service_name
                for pattern in rule_services
            )
    ...
    async def get_handler_info(
        ...
        async def _get_handler_info_operation():
            ...
            for handler in handlers:
                info = {
                    "name": handler.name,
                    "description": getattr(
                        handler, "description", f"{handler.name} parameter handler"
                    ),
        ...
        return await self._execute_with_error_handling(
            _get_handler_info_operation, f"get_handler_info_{service_name}"
        )
    ...
    async def get_specialized_defaults(
        ...
        async def _get_specialized_defaults_operation():
            ...
            try:
                ...
                return {
                    ...
                    "handler_messages": (
                        [msg.message for msg in handler_result.validation_messages]
                        if handler_result.validation_messages
                        else []
                    ),
                ...
                self.logger.error(
                    f"Handler {handler.name} failed for service {service_name}: {e}"
                )
                return {
                    "service_name": service_name,
                    "handler_used": handler.name,
        ...
        return await self._execute_with_error_handling(
            _get_specialized_defaults_operation,
            f"get_specialized_defaults_{service_name}",
        )
    ...
    async def validate_with_handler(
        ...
        async def _validate_with_handler_operation():
            ...
            if not handler:
                return {
                    ...
                    "is_valid": False,
                    "errors": [
                        f"No specialized handler found for service: {service_name}"
                    ],
                    "warnings": [],
            ...
            try:
                handler_result = handler.validate_parameters(
                    parameters, service_name, context
                )
                ...
                self.logger.error(
                    f"Handler {handler.name} validation failed for service {service_name}: {e}"
                )
                return {
                    "service_name": service_name,
                    "handler_used": handler.name,
        ...
        return await self._execute_with_error_handling(
            _validate_with_handler_operation, f"validate_with_handler_{service_name}"
        )
    ...
    async def get_parameter_suggestions(
        ...
        async def _get_suggestions_operation():
            ...
            try:
                suggestions = handler.suggest_parameters(
                ...
                self.logger.error(
                    f"Handler {handler.name} suggestions failed for service {service_name}: {e}"
                )
                return []

        return await self._execute_with_error_handling(
            _get_suggestions_operation, f"get_parameter_suggestions_{service_name}"
        )
    ...
    async def list_available_handlers(self) -> ServiceResult[Dict[str, Any]]:
        ...
        return await self._execute_with_error_handling(
            _list_handlers_operation, "list_available_handlers"
        )
