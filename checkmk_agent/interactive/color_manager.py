...
class ColorManager:
    ...
    def _detect_terminal_capabilities(self) -> Dict[str, bool]:
        ...
        # 256-color support
        if ('256' in term or '256' in colorterm or 
            term in ['xterm-256color', 'screen-256color'] or
            colorterm in ['gnome-terminal', 'xfce4-terminal']):
            capabilities['color_256'] = True
        
        # RGB/truecolor support
        if (colorterm in ['truecolor', '24bit'] or
            'truecolor' in term):
    ...
    def preview_colors(self) -> str:
        ...
        color_groups = [
            ...
            ("Extended", ['orange', 'purple', 'pink', 'gray', 'dark_gray', 'light_gray']),
        ]
    ...
    def test_colors(self) -> str:
        ...
        test_lines.append(f"  {self.get_message_icon(MessageType.INFO)} {self.colorize('This is an info message about system status', self.get_message_color(MessageType.INFO))}")
        test_lines.append(f"  {self.get_message_icon(MessageType.SUCCESS)} {self.colorize('Operation completed successfully!', self.get_message_color(MessageType.SUCCESS))}")
        test_lines.append(f"  {self.get_message_icon(MessageType.WARNING)} {self.colorize('Warning: This action may take some time', self.get_message_color(MessageType.WARNING))}")
        test_lines.append(f"  {self.get_message_icon(MessageType.ERROR)} {self.colorize('Error: Unable to connect to server', self.get_message_color(MessageType.ERROR))}")
        test_lines.append(f"  {self.get_message_icon(MessageType.HELP)} {self.colorize('Type help for more information', self.get_message_color(MessageType.HELP))}")
        test_lines.append("")
        ...
        test_lines.append(f"  {prompt_icon} {self.colorize('checkmk', prompt_color)}> list hosts")
        test_lines.append("")
