...
"""
    ...
    def format_service_health_dashboard(self, dashboard: Dict[str, Any]) -> str:
        ...
        if urgent_problems:
            ...
            result += f"   Use {self.colorize('status critical', 'bold')} for details\n\n"
        
        unhandled = dashboard.get('needs_attention', 0)
        if unhandled > 0:
            result += f"💡 {self.colorize(f'{unhandled} unacknowledged problem(s)', 'yellow')} need review\n"
            result += f"   Use {self.colorize('status problems', 'bold')} to see all issues\n"
    ...
    def format_service_status_card(self, service: Dict[str, Any]) -> str:
        ...
        # Build card
        card = f"┌─ {state_icon} {self.colorize(f'{host_name}/{description}', 'bold')} ─\n"
    ...
    def format_problem_summary(self, problems: List[Dict[str, Any]], title: str = "Service Problems") -> str:
    ...
    def format_host_status_dashboard(self, dashboard: Dict[str, Any]) -> str:
        ...
        if not dashboard.get('found', True):
            return self.format_message(dashboard.get('error', 'Host not found'), MessageType.ERROR)
        ...
        result += f"   🟢 OK: {self.colorize(str(state_counts.get('ok', 0)), 'green')}\n\n"
        
        # Urgent Issues Section
        if urgent_issues:
            result += f"{self.colorize('🚨 Urgent Issues Requiring Attention', 'red')}\n"
            for issue in urgent_issues[:3]:  # Show top 3
                ...
                result += f"     💡 {self.colorize(issue['recommended_action'], 'cyan')}\n"
            result += "\n"
