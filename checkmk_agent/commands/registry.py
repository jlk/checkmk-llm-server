...
class CommandRegistry:
    ...
    def register(self, command: BaseCommand, aliases: Optional[List[str]] = None) -> 'CommandRegistry':
        ...
        if command.name in self._aliases:
            raise ValueError(f"Command name '{command.name}' conflicts with existing alias")
        
        # Check for alias conflicts
        all_aliases = (aliases or []) + command.aliases
        for alias in all_aliases:
            if alias in self._commands:
                raise ValueError(f"Alias '{alias}' conflicts with existing command name")
        ...
        self.logger.debug(f"Registered command '{command.name}' with {len(all_aliases)} aliases")
        return self
    ...
    def unregister(self, command_name: str) -> bool:
        ...
        # Remove aliases
        aliases_to_remove = [alias for alias, cmd_name in self._aliases.items() 
                           if cmd_name == command_name]
        for alias in aliases_to_remove:
            del self._aliases[alias]
        
        self.logger.debug(f"Unregistered command '{command_name}' and {len(aliases_to_remove)} aliases")
        return True
    ...
    def list_commands(self, category: Optional[CommandCategory] = None) -> List[BaseCommand]:
    ...
    def list_command_names(self, category: Optional[CommandCategory] = None) -> List[str]:
    ...
    def validate_registry(self) -> List[str]:
        ...
        for alias, command_name in self._aliases.items():
            if command_name not in self._commands:
                errors.append(f"Alias '{alias}' points to non-existent command '{command_name}'")
        
        # Check for category consistency
        for category, command_names in self._categories.items():
            for command_name in command_names:
                if command_name not in self._commands:
                    errors.append(f"Category '{category.value}' contains non-existent command '{command_name}'")
