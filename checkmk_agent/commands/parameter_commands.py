...
class ViewDefaultParametersCommand(ParameterCommand):
    ...
    def execute(self, context: CommandContext) -> CommandResult:
        ...
        try:
            ...
            result_message = (
                f"📊 Default Parameters for {service_type.upper()} services:\n\n"
            )
            ...
            if "average" in default_params:
                result_message += (
                    f"📈 Averaging Period: {default_params['average']} minutes\n"
                )

            if "magic_normsize" in default_params:
                result_message += (
                    f"💾 Magic Normsize: {default_params['magic_normsize']} GB\n"
                )
...
class ViewServiceParametersCommand(ParameterCommand):
    ...
    def execute(self, context: CommandContext) -> CommandResult:
        ...
        try:
            ...
            params = self.parameter_manager.get_service_parameters(
            ...
            if "parameters" in params:
                ...
                if "magic_normsize" in param_data:
                    result_message += (
                        f"💾 Magic Normsize: {param_data['magic_normsize']} GB\n"
                    )
...
class SetServiceParametersCommand(ParameterCommand):
    ...
    def _custom_validate(self, context: CommandContext) -> Optional[CommandResult]:
        ...
        if warning_value is not None and (warning_value < 0 or warning_value > 100):
            return CommandResult.error_result("Warning value must be between 0 and 100")
    ...
    def execute(self, context: CommandContext) -> CommandResult:
        ...
        try:
            ...
            if not comment:
                comment = (
                    f"Override thresholds for {service_description} on {host_name}"
                )
            ...
            result_message += (
                "⏱️  Changes will take effect after next service check cycle"
            )
...
class CreateParameterRuleCommand(ParameterCommand):
    ...
    def execute(self, context: CommandContext) -> CommandResult:
        ...
        try:
            ...
            result_message = (
                f"📋 To create a parameter rule for {service_type} services:\n\n"
            )
            ...
            result_message += (
                f"Example: 'set {service_type} warning to 85% for server01'"
            )
    ...
    def execute(self, context: CommandContext) -> CommandResult:
        ...
        try:
            ...
            if not ruleset_name:
                ...
                result_message = (
                    f"📋 Available Parameter Rulesets ({len(rulesets)}):\n\n"
                )
...
class DeleteParameterRuleCommand(ParameterCommand):
    ...
    def execute(self, context: CommandContext) -> CommandResult:
        ...
        try:
            ...
            result_message += (
                "⏱️  Changes will take effect after configuration activation"
            )
