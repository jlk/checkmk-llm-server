            ...
            if host_name:
                ...
                if not services:
                    return CommandResult.success_result(
                        data=[], 
                        message=f"📦 No services found for host: {host_name}"
                    )
                
                # Format results
                result_message = f"📦 Found {len(services)} services for host: {host_name}\n\n"
...
class AcknowledgeServiceCommand(ServiceCommand):
    ...
    def execute(self, context: CommandContext) -> CommandResult:
        ...
        try:
            ...
            return CommandResult.success_result(
                data={'host_name': host_name, 'service_description': service_desc, 'comment': comment},
                message=result_message
            )
