...
class ServiceParameterManager:
    ...
    PARAMETER_RULESETS = {
        ...
        "filesystem": {"all": "filesystems", "default": "filesystems"},
        "disk": {"io": "disk_io", "stats": "diskstat", "default": "disk_io"},
        "network": {
    ...
    DEFAULT_TEMPLATES = {
        "cpu_utilization_linux": {"levels": (80.0, 90.0), "average": 15, "horizon": 90},
        "cpu_utilization_simple": {"levels": (80.0, 90.0), "average": 15},
        "memory_linux": {"levels": (80.0, 90.0), "average": 3, "handle_zero": True},
        "memory_level_windows": {"levels": (80.0, 90.0), "average": 3},
        "filesystems": {
            "levels": (80.0, 90.0),
            "magic_normsize": 20,
            "magic": 0.8,
        ...
        "interfaces": {"speed": "auto", "levels": (80.0, 90.0), "unit": "percent"},
    }
    ...
    def get_default_parameters(
        self, service_type: str, os_type: str = "linux"
    ) -> Dict[str, Any]:
        """
        ...
        if not ruleset_name:
            self.logger.warning(
                f"No default ruleset found for service type: {service_type}"
            )
            return {}
        ...
        try:
            ...
            if comment:
                ...
                properties["description"] = (
                    f"Custom parameters for {service_pattern} on {host_name}"
                )
            ...
            self.logger.info(
                f"Created parameter rule {rule_id} for {host_name}/{service_pattern}"
            )
            return rule_id
    ...
    def update_service_parameters(
        self, rule_id: str, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.warning(
                "Rule update not directly supported, will recreate rule"
            )
    ...
    def get_service_parameters(
        ...
        try:
            ...
            affecting_rules = self.checkmk_client.search_rules_by_host_service(
                host_name, service_name
            )

            if not affecting_rules:
                return {"parameters": {}, "source": "default", "rules": []}

            # Sort rules by precedence (most specific first)
            sorted_rules = self._sort_rules_by_precedence(
                affecting_rules, host_name, service_name
            )
            ...
            self.logger.error(
                f"Error getting service parameters for {host_name}/{service_name}: {e}"
            )
            raise
        ...
        try:
            ...
            if ruleset == "filesystems":
                if "magic_normsize" in parameters:
                    if (
                        not isinstance(parameters["magic_normsize"], (int, float))
                        or parameters["magic_normsize"] <= 0
                    ):
                        return False
                if "magic" in parameters:
                    if not isinstance(parameters["magic"], (int, float)) or not (
                        0 < parameters["magic"] <= 1
                    ):
                        return False
            ...
            if "cpu_utilization" in ruleset:
                if "average" in parameters:
                    if (
                        not isinstance(parameters["average"], (int, float))
                        or parameters["average"] <= 0
                    ):
                        return False
    ...
    def discover_service_ruleset(
        ...
        if "cpu" in service_lower or "processor" in service_lower:
            ...
            "filesystem" in service_lower
            or "disk" in service_lower
            or "mount" in service_lower
        ):
            return self.PARAMETER_RULESETS["filesystem"]["default"]
        elif (
            "interface" in service_lower
            or "network" in service_lower
            or "eth" in service_lower
        ):
    ...
    def create_simple_override(
        ...
        parameters = self.get_default_parameters(
            "cpu" if "cpu_utilization" in ruleset else "filesystem"
        )
    ...
    def _sort_rules_by_precedence(
        self, rules: List[Dict[str, Any]], host_name: str, service_name: str
    ) -> List[Dict[str, Any]]:
        """
