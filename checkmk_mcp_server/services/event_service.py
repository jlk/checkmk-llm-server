...
class EventInfo:
    ...
    def __init__(self, event_data: Dict[str, Any]):
        ...
        self.service_description = event_data.get("extensions", {}).get(
            "service_description", ""
        )
...
class EventService(BaseService):
    ...
    async def list_events(
        ...
        return await self._execute_with_error_handling(
            _list_events_operation, "list_events"
        )
    ...
    async def get_event(
        ...
        return await self._execute_with_error_handling(
            _get_event_operation, f"get_event_{event_id}"
        )
        ...
        async def _list_service_events_operation():
            ...
            if service_name and service_name.strip():
                query["expr"].append(
                    {
                        "op": "~",
                        "left": "eventconsoleevents.event_text",
                        "right": service_name,
                    }
                )
                    ...
                    query["expr"].append(
                        {
                            ...
                            "right": state_map[state_filter.lower()],
                        }
                    )
    ...
    async def list_host_events(
        self, host_name: str, limit: int = 100, state_filter: Optional[str] = None
    ) -> ServiceResult[List[EventInfo]]:
        """
        ...
        return await self._execute_with_error_handling(
            _list_host_events_operation, f"list_host_events_{host_name}"
        )
    ...
    async def get_recent_critical_events(
        ...
        return await self._execute_with_error_handling(
            _get_recent_critical_events_operation, "get_recent_critical_events"
        )
    ...
    async def acknowledge_event(
        ...
        return await self._execute_with_error_handling(
            _acknowledge_event_operation, f"acknowledge_event_{event_id}"
        )
    ...
    async def change_event_state(
        ...
        async def _change_event_state_operation():
            ...
            if new_state.lower() not in state_map:
                raise ValueError(
                    f"Invalid state: {new_state}. Must be one of: {list(state_map.keys())}"
                )

            data = {"new_state": state_map[new_state.lower()]}
            if comment:
                data["comment"] = comment
            if site_id:
                data["site_id"] = site_id
        ...
        return await self._execute_with_error_handling(
            _change_event_state_operation, f"change_event_state_{event_id}"
        )
    ...
    async def search_events(
        ...
        async def _search_events_operation():
            ...
            # Use the API client's list_events method directly
            sync_client = self.checkmk.sync_client
            events_data = sync_client.list_events(
                query=query, host=host_filter, state=state_filter
            )
        ...
        return await self._execute_with_error_handling(
            _search_events_operation, f"search_events_{search_term}"
        )
