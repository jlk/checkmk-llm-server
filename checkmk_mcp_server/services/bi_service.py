...
class BIAggregation:
    ...
    def __init__(self, aggregation_id: str, aggregation_data: Dict[str, Any]):
        ...
        self.scheduled_downtime_depth = aggregation_data.get(
...
class BIService(BaseService):
    ...
    async def get_bi_aggregation_states(
        ...
        return await self._execute_with_error_handling(
            _get_bi_aggregation_states_operation, "get_bi_aggregation_states"
        )

    async def get_bi_aggregation_state(
        self, aggregation_name: str
        ...
        async def _get_bi_aggregation_state_operation():
            result = await self.get_bi_aggregation_states(
                filter_names=[aggregation_name]
            )
            if not result.success:
                raise CheckmkAPIError(
                    f"Failed to get BI aggregation state: {result.error}"
                )
    ...
    async def list_bi_packs(self) -> ServiceResult[List[BIPack]]:
        ...
        return await self._execute_with_error_handling(
            _list_bi_packs_operation, "list_bi_packs"
        )

    async def get_business_status_summary(
        self, filter_groups: Optional[List[str]] = None
        ...
        async def _get_business_status_summary_operation():
            ...
            if not result.success:
                raise CheckmkAPIError(
                    f"Failed to get BI aggregation states: {result.error}"
                )
        ...
        return await self._execute_with_error_handling(
            _get_business_status_summary_operation, "get_business_status_summary"
        )
    ...
    async def get_critical_business_services(
        ...
        return await self._execute_with_error_handling(
            _get_critical_business_services_operation, "get_critical_business_services"
        )
