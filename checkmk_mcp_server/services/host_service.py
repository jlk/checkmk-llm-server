...
from .models.hosts import (
    ...
    HostUpdateResult,
    HostDeleteResult,
    HostBulkCreateResult,
    HostBulkDeleteResult,
    HostState,
)
...
class HostService(BaseService):
    ...
    async def list_hosts(
        ...
        Args:
            ...
            effective_attributes: Whether to include effective attributes
                (permissions enforced by Checkmk)
    ...
    async def create_host(
        ...
        async def _create_operation():
            ...
            validation_errors = self._validate_create_host_params(
                name, folder, ip_address
            )
        ...
        return await self._execute_with_error_handling(
            _create_operation, f"create_host_{name}"
        )
    ...
    async def update_host(
        ...
        return await self._execute_with_error_handling(
            _update_operation, f"update_host_{name}"
        )
    ...
    async def delete_host(self, name: str) -> ServiceResult[HostDeleteResult]:
        ...
        return await self._execute_with_error_handling(
            _delete_operation, f"delete_host_{name}"
        )

    async def get_host(
        self, name: str, include_status: bool = True, effective_attributes: bool = False
    ) -> ServiceResult[HostInfo]:
        """
        ...
        async def _get_operation():
            ...
            host_data = await self.checkmk.get_host(
                name, effective_attributes=effective_attributes
            )
        ...
        return await self._execute_with_error_handling(
            _get_operation, f"get_host_{name}"
        )

    async def bulk_create_hosts(
        self, hosts_data: List[Dict[str, Any]]
    ) -> ServiceResult[HostBulkCreateResult]:
        """
            ...
            for i, host_data in enumerate(hosts_data):
                ...
                if errors:
                    validation_errors.extend(
                        [f"Host {i+1}: {error}" for error in errors]
                    )
        ...
        return await self._execute_with_error_handling(
            _bulk_create_operation, "bulk_create_hosts"
        )

    def _convert_api_host_to_model(
        self, host_data: Dict[str, Any], include_status: bool = False
    ) -> HostInfo:
        ...
        if include_status and "status" in extensions:
            ...
            if status.get("last_state_change"):
                host_info.last_state_change = datetime.fromtimestamp(
    ...
    def _apply_host_filters(
        self, hosts: List[HostInfo], search: Optional[str], folder: Optional[str]
    ) -> List[HostInfo]:
        ...
        if search:
            filtered_hosts = self._apply_text_filter(
                filtered_hosts, search, ["name", "ip_address", "attributes.alias"]
            )
    ...
    def _validate_create_host_params(
        self, name: str, folder: str, ip_address: Optional[str]
    ) -> List[str]:
