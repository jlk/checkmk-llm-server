...
class StatusService(BaseService):
    ...
    async def get_health_dashboard(self) -> ServiceResult[HealthDashboard]:
        ...
        async def _dashboard_operation():
            ...
            overall_health = self._calculate_health_percentage(
            ...
            recommendations = self._generate_health_recommendations(
                problem_summary, critical_problems
            )
        ...
        return await self._execute_with_error_handling(
            _dashboard_operation, "get_health_dashboard"
        )
    ...
    async def list_problems(
        ...
        async def _list_problems_operation():
            ...
            # Apply filters
            filtered_problems = self._apply_problem_filters(
                problems, severity, category
            )

            # Sort by urgency/severity
            filtered_problems.sort(
                key=lambda p: (p.urgency_score, p.severity), reverse=True
            )
        ...
        return await self._execute_with_error_handling(
            _list_problems_operation, "list_problems"
        )

    async def query_status(
        self, filters: StatusFilterOptions
    ) -> ServiceResult[StatusQueryResult]:
        """
        ...
        async def _query_operation():
            ...
            if filters.state_filter and ServiceState.OK not in filters.state_filter:
                ...
                service_data = await self.checkmk.list_problem_services(
                    filters.host_filter
                )
            else:
            ...
            # Sort results
            sorted_problems = self._sort_problems(
                filtered_problems, filters.sort_by, filters.sort_order
            )
    ...
    def _convert_service_to_problem(
    ...
    def _identify_urgent_problems(
        ...
        urgent_services = [s for s in problem_services if self._is_service_urgent(s)]
    ...
    async def _calculate_host_statuses(
    ...
    def _categorize_service_problem(
        self, service_data: Dict[str, Any]
    ) -> ProblemCategory:
    ...
    def _apply_comprehensive_filters(
        self, problems: List[ServiceProblem], filters: StatusFilterOptions
    ) -> List[ServiceProblem]:
        ...
        if filters.service_filter:
            filtered = self._apply_text_filter(
                filtered, filters.service_filter, ["service_name"]
            )

        # Time-based filters
        if filters.problems_since:
            filtered = [
                p for p in filtered if p.last_state_change >= filters.problems_since
            ]

        # Acknowledgment filter
        if filters.acknowledged_filter is not None:
            filtered = [
                p for p in filtered if p.acknowledged == filters.acknowledged_filter
            ]
    ...
    def _sort_problems(
        self, problems: List[ServiceProblem], sort_by: str, sort_order: str
    ) -> List[ServiceProblem]:
        ...
        if sort_by == "urgency":
            ...
            priority_map = {
                ...
                ProblemSeverity.WARNING: 2,
                ProblemSeverity.UNKNOWN: 1,
            }
            return sorted(
                problems, key=lambda p: priority_map.get(p.severity, 0), reverse=reverse
            )
        elif sort_by == "duration":
            return sorted(
                problems, key=lambda p: p.last_state_change, reverse=not reverse
            )  # Older problems first for asc
        elif sort_by == "host":
    ...
    def _generate_health_recommendations(
        self, problem_summary: ProblemSummary, critical_problems: List[ServiceProblem]
    ) -> List[str]:
        ...
        if critical_problems:
            recommendations.append(
                f"Address {len(critical_problems)} critical service(s) immediately"
            )

        if problem_summary.unacknowledged_problems > 5:
            recommendations.append(
                "Review and acknowledge known issues to reduce noise"
            )
        ...
        if categories.get(ProblemCategory.DISK.value, 0) > 3:
            recommendations.append(
                "Multiple disk issues detected - consider storage maintenance"
            )
    ...
    def _generate_health_alerts(
        self,
        critical_problems: List[ServiceProblem],
        urgent_problems: List[ServiceProblem],
    ) -> List[str]:
        ...
        if len(critical_problems) > 10:
            alerts.append(
                "⚠️ Large number of critical services - possible infrastructure issue"
            )
        ...
        if urgent_problems:
            ...
            if len(hosts_with_problems) == 1:
                alerts.append(
                    f"⚠️ All urgent problems on single host: {list(hosts_with_problems)[0]}"
                )
    ...
    def _calculate_health_trend(self, problem_summary: ProblemSummary) -> str:
        ...
        if (
            problem_summary.new_problems_last_hour
            > problem_summary.resolved_problems_last_hour
        ):
            return "degrading"
        elif (
            problem_summary.resolved_problems_last_hour
    ...
    async def get_critical_problems(
        ...
        category = (
            category_filter[0] if category_filter and len(category_filter) > 0 else None
        )

        problems_result = await self.list_problems(severity=severity, category=category)
        ...
        if problems_result.success:
            ...
            summary = {
                ...
                "warning_problems": len(
                    [p for p in problems if p.severity == ProblemSeverity.WARNING]
                ),
                "problems": [
                    {
    ...
    async def analyze_host_health(
        ...
        async def _analyze_operation():
            ...
            if include_recommendations:
                result["recommendations"] = self._generate_host_recommendations(
                    states, problems
                )
            ...
            if compare_to_peers:
                result["peer_comparison"] = {
                    ...
                    "ranking": (
                        "Above Average" if health_percentage > 85 else "Below Average"
                    ),
                }
    ...
    async def get_host_problems(
        self, host_name: str, include_services: bool = True
    ) -> ServiceResult[Dict[str, Any]]:
        ...
        if problems_result.success:
            ...
            result = {
                ...
                "problems_by_severity": {
                    ...
                    "unknown": len(
                        [p for p in problems if p.severity == ProblemSeverity.UNKNOWN]
                    ),
                },
            }
    ...
    async def get_infrastructure_summary(
        self, include_trends: bool = True, time_range_hours: int = 24
    ) -> ServiceResult[Dict[str, Any]]:
    ...
    async def get_problem_trends(
        self, time_range_hours: int = 24, category_breakdown: bool = True
    ) -> ServiceResult[Dict[str, Any]]:
    ...
    def _generate_host_recommendations(
        self, states: Dict[str, int], problems: List[str]
    ) -> List[str]:
        ...
        if states["critical"] > 0:
            recommendations.append(
                f"Address {states['critical']} critical service(s) immediately"
            )

        if states["warning"] > 5:
            recommendations.append(
                "High number of warnings - schedule maintenance window"
            )

        if states["unknown"] > 0:
            recommendations.append(
                "Investigate unknown service states - possible monitoring issues"
            )

        # Pattern detection
        if any("disk" in p.lower() or "filesystem" in p.lower() for p in problems):
            recommendations.append(
                "Disk-related issues detected - check storage capacity"
            )
