...
class ServiceOperationsFacade:
    ...
    def process_command(self, user_input: str) -> str:
        ...
        try:
            ...
            self.logger.debug(
                f"Command analysis: {analysis.action} from {analysis.source}"
            )

            # Normalize action using backward compatibility mapping
            normalized_action = self._action_mapping.get(
    ...
    def _handle_unknown_command(
        self, original_action: str, normalized_action: str
    ) -> str:
        ...
        suggestions = self.registry.find_similar_commands(
            original_action, max_suggestions=3
        )
    ...
    def validate_system(self) -> Dict[str, Any]:
        ...
        validation_results["is_valid"] = (
            len(validation_results["registry_errors"]) == 0
            and len(validation_results["factory_errors"]) == 0
        )
    ...
    def __repr__(self) -> str:
        return (
            f"ServiceOperationsFacade(commands={len(self.registry)}, "
