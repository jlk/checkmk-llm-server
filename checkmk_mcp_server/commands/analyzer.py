...
class LLMResponseValidator:
    ...
    VALID_ACTIONS = [
        ...
        "view_default_parameters",
        "show_default_parameters",
        "default_parameters",
        "view_service_parameters",
        "show_service_parameters",
        "service_parameters",
        "show_parameters",
...
class PatternMatcher:
    ...
    def __init__(self):
        self.patterns = [
            # Service operations
            (
                r"list services(?:\s+(?:for|on)\s+(\w+))?",
                "list_services",
                {"host_name": 1},
            ),
            (
                r"show services(?:\s+(?:for|on)\s+(\w+))?",
                "list_services",
                {"host_name": 1},
            ),
            (
                r"get services(?:\s+(?:for|on)\s+(\w+))?",
                "list_services",
                {"host_name": 1},
            ),
            (
                r"(?:check|status)\s+(.+?)\s+on\s+(\w+)",
                "get_service_status",
            ...
            (
                r"discover\s+services\s+on\s+(\w+)",
                "discover_services",
                {"host_name": 1},
            ),
            ...
            # Parameter operations
            (
                r"show\s+default\s+(\w+)\s+parameters",
                "view_default_parameters",
                {"service_type": 1},
            ),
            (
                r"(?:show|view)\s+parameters\s+for\s+(.+?)\s+on\s+(\w+)",
                "view_service_parameters",
                {"service_description": 1, "host_name": 2},
            ),
            (
                r"set\s+(\w+)\s+warning\s+to\s+(\d+)%?\s+for\s+(\w+)",
                "set_service_parameters",
                {
            ...
            (
                r"set\s+(\w+)\s+critical\s+to\s+(\d+)%?\s+for\s+(\w+)",
                "set_service_parameters",
                {
                    "service_description": 1,
                    "critical_value": 2,
                    "host_name": 3,
                    "parameter_type": "critical",
                },
            ),
            # Utility operations
        ...
        for pattern, action, param_mapping in self.patterns:
            ...
            if match:
                ...
                for param_name, group_index in param_mapping.items():
                    if isinstance(group_index, int) and group_index <= len(
                        match.groups()
                    ):
