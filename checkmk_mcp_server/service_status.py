...
class ServiceStatusManager:
    ...
    def get_service_health_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            ...
            dashboard = {
                ...
                "needs_attention": len(
                    [p for p in problem_services if not self._is_service_handled(p)]
                ),
                "summary_message": self._generate_health_message(
                    health_summary, problem_analysis
                ),
            }

            self.logger.info(
                f"Generated service health dashboard with {health_summary['total_services']} services"
            )
            return dashboard
    ...
    def analyze_service_problems(
        ...
        try:
            ...
            recommendations = self._generate_problem_recommendations(
                problem_services, categories
            )
            ...
            analysis = {
                ...
                "critical_count": len(
                    [s for s in problem_services if self._get_service_state(s) == 2]
                ),
                "warning_count": len(
                    [s for s in problem_services if self._get_service_state(s) == 1]
                ),
                "unhandled_count": len(
                    [s for s in problem_services if not self._is_service_handled(s)]
                ),
            }
    ...
    def get_service_status_details(
        ...
        try:
            # Get service status
            service_info = self.checkmk_client.get_service_status(
            ...
            if service_description is None:
                ...
                health_pct = (
                    (state_counts["ok"] / total_services * 100)
                    if total_services > 0
                    else 100.0
                )
            ...
            status_analysis = {
                ...
                "requires_action": state != 0
                and acknowledged == 0
                and downtime_depth == 0,
            }
            ...
            self.logger.info(
                f"Retrieved detailed status for {host_name}/{service_description}"
            )
            return details
    ...
    def get_host_status_dashboard(self, host_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            dashboard = {
                ...
                "health_metrics": {
                    ...
                    "health_trend": self._calculate_health_trend(
                        health_percentage, infrastructure_avg
                    ),
            ...
            self.logger.info(
                f"Generated enhanced host status dashboard for {host_name}"
            )
            return dashboard

        except CheckmkAPIError as e:
            self.logger.error(
                f"Error generating host status dashboard for {host_name}: {e}"
            )
            raise

    def find_services_by_criteria(
        ...
        try:
            ...
            if criteria.get("state") is not None:
                services = self.checkmk_client.get_services_by_state(
                    criteria["state"], criteria.get("host_filter")
                )
            ...
            if criteria.get("output_contains"):
                ...
                services = [
                    ...
                    for s in services
                    if output_filter
                    in s.get("extensions", {}).get("plugin_output", "").lower()
                ]
    ...
    def _analyze_problems_by_severity(
        self, problem_services: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _group_problems_by_host(
        self, problem_services: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Group problems by host for host-centric view."""
        host_problems = defaultdict(
            lambda: {"critical": 0, "warning": 0, "unknown": 0, "total": 0}
        )
    ...
    def _identify_urgent_problems(
        ...
        urgent_problems.sort(
            key=lambda x: (x["state"], x["urgency_score"]), reverse=True
        )
    ...
    def _categorize_problems(
        ...
        for service in problem_services:
            ...
            if any(
                keyword in description for keyword in ["disk", "filesystem", "storage"]
            ):
                categories["disk_problems"].append(service_key)
            elif any(
                keyword in description for keyword in ["network", "interface", "ping"]
            ):
                categories["network_problems"].append(service_key)
            elif any(
                keyword in description
                for keyword in ["cpu", "memory", "load", "performance"]
            ):
                categories["performance_issues"].append(service_key)
            elif any(
                keyword in output for keyword in ["connection", "timeout", "refused"]
            ):
    ...
    def _generate_problem_recommendations(
        self, problem_services: List[Dict[str, Any]], categories: Dict[str, List[str]]
    ) -> List[str]:
        ...
        if disk_count > 0:
            recommendations.append(
                f"💾 {disk_count} disk/storage issue(s) - check disk space and performance"
            )

        if network_count > 0:
            recommendations.append(
                f"🌐 {network_count} network issue(s) - verify connectivity and interface status"
            )
        ...
        if unhandled_count > 0:
            recommendations.append(
                f"⚠️  {unhandled_count} unacknowledged problem(s) need review"
            )

        if len(problem_services) > 10:
            recommendations.append(
                "📊 Consider using filters to focus on specific hosts or service types"
            )
    ...
    def _identify_recurring_issues(
        ...
        for service in problem_services:
            ...
            if any(
                keyword in output
                for keyword in ["flapping", "intermittent", "unstable"]
            ):
    ...
    def _generate_health_message(
        self, health_summary: Dict[str, Any], problem_analysis: Dict[str, Any]
    ) -> str:
    ...
    def _is_service_handled(self, service: Dict[str, Any]) -> bool:
        ...
        extensions = service.get("extensions", {})
        return (
            extensions.get("acknowledged", 0) > 0
            or extensions.get("scheduled_downtime_depth", 0) > 0
        )
    ...
    def _analyze_host_problems_by_category(
        self, problem_services: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        for service in problem_services:
            ...
            if any(
                keyword in description
                for keyword in ["disk", "filesystem", "storage", "mount", "space"]
            ):
                categories["disk_problems"].append(service_info)
                category_counts["disk"] += 1
            elif any(
                keyword in description
                for keyword in ["network", "interface", "ping", "port", "tcp", "udp"]
            ):
                categories["network_problems"].append(service_info)
                category_counts["network"] += 1
            elif any(
                keyword in description
                for keyword in ["cpu", "memory", "load", "performance", "utilization"]
            ):
                ...
                keyword in description
                for keyword in ["check_mk", "agent", "monitoring", "snmp"]
            ):
    ...
    def _identify_host_urgent_issues(
        self, problem_services: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
                ...
                urgent_issues.append(
                    {
                        ...
                        "recommended_action": self._get_service_recommended_action(
                            service
                        ),
                    }
                )
        ...
        urgent_issues.sort(
            key=lambda x: (
                self.STATE_PRIORITIES.get(x["state"], 0),
                x["urgency_score"],
            ),
            reverse=True,
        )
    ...
    def _generate_host_maintenance_recommendations(
        self, problem_services: List[Dict[str, Any]], problem_analysis: Dict[str, Any]
    ) -> List[str]:
        ...
        if category_counts.get("disk", 0) > 0:
            recommendations.append(
                f"💾 Disk issues detected - check filesystem usage and disk health"
            )

        if category_counts.get("performance", 0) > 0:
            recommendations.append(
                f"⚡ Performance issues detected - monitor CPU/memory usage"
            )

        if category_counts.get("network", 0) > 0:
            recommendations.append(
                f"🌐 Network issues detected - verify interface configuration and connectivity"
            )

        if category_counts.get("connectivity", 0) > 0:
            recommendations.append(
                f"🔌 Connectivity issues detected - check service dependencies and firewall rules"
            )
        ...
        if len(problem_services) > 5:
            recommendations.append(
                "📋 Consider scheduling maintenance window for comprehensive fixes"
            )

        if category_counts.get("monitoring", 0) > 0:
            recommendations.append(
                "🔍 Monitoring system issues detected - check agent connectivity"
            )

        return recommendations

    def _analyze_recent_service_changes(
    ...
    def _generate_host_summary_message(
        self,
        health_percentage: float,
        problem_count: int,
        urgent_issues: List[Dict[str, Any]],
    ) -> str:
