...
class CheckmkConfig(BaseModel):
    ...
    auto_activate_changes: bool = Field(
        default=True,
        description="Automatically activate changes after rule modifications",
    )
...
class UIConfig(BaseModel):
    ...
    auto_detect_terminal: bool = Field(
        default=True, description="Auto-detect terminal capabilities"
    )
...
def load_config(config_file: Optional[Union[str, Path]] = None) -> AppConfig:
    ...
    env_config = {
        "checkmk": {
            ...
            "auto_activate_changes": os.getenv("AUTO_ACTIVATE_CHANGES"),
        },
        "llm": {
    ...
    checkmk_config = CheckmkConfig(
        ...
        auto_activate_changes=bool(checkmk_data.get("auto_activate_changes", True)),
    )
    ...
    if isinstance(auto_detect_terminal, str):
        auto_detect_terminal = auto_detect_terminal.lower() in (
            "true",
            "1",
            "yes",
            "on",
