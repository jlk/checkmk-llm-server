...
class TabCompleter:
    ...
    def __init__(self, checkmk_client=None, help_system=None):
        ...
        self.theme_commands = ["theme list", "theme current", "theme set"]
        ...
        self.color_commands = ["colors show", "colors test", "colors terminal"]
        ...
        self.service_related = ["services", "service", "svc"]
        ...
        self.prepositions = ["on", "for", "in", "with", "from", "to", "of", "at", "by"]
    ...
    def _complete_contextual(self, tokens: List[str], text: str) -> List[str]:
        ...
        if lower_tokens[0] == "theme":
            if len(tokens) == 1:
                ...
                return [
                    cmd for cmd in ["list", "current", "set"] if cmd.startswith(text)
                ]
        ...
        if lower_tokens[0] in ["colors", "color"]:
            if len(tokens) == 1:
                ...
                return [
                    cmd for cmd in ["show", "test", "terminal"] if cmd.startswith(text)
                ]
    ...
    def _get_host_names(self, prefix: str = "") -> List[str]:
        ...
        try:
            ...
            if (
                self._host_cache is None
                or current_time - self._last_cache_time > self._cache_timeout
            ):
    ...
    def _get_service_names(self, host_name: str, prefix: str = "") -> List[str]:
        ...
        try:
            ...
            cache_key = host_name
            if (
                cache_key not in self._service_cache
                or current_time - self._service_cache[cache_key]["time"]
                > self._cache_timeout
            ):
            ...
            if prefix:
                return [
                    svc for svc in services if svc.lower().startswith(prefix.lower())
                ]
            else:
                return services
