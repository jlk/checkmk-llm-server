...
class CommandParser:
    ...
    def parse_command(self, user_input: str) -> CommandIntent:
        ...
        if user_input.lower().startswith("colors") or user_input.lower().startswith(
            "color "
        ):
            return CommandIntent(
                command=user_input.lower(),
    ...
    def _extract_command(self, tokens: List[str]) -> Tuple[str, float]:
        ...
        all_commands = set(self.command_aliases.keys()) | set(
            self.command_aliases.values()
        )
        ...
        has_action_word = any(
            word in text
            for word in ["show", "list", "display", "get", "find", "search"]
        )
        ...
        if not has_action_word:
            ...
            if (
                any(word in text for word in ["health", "status", "problems", "issues"])
                and len(tokens) <= 2
            ):
                return "status", 0.8
        ...
        if any(word in text for word in ["host", "server", "machine"]):
            if any(
                ...
                word in text for word in ["status", "health", "problems", "issues"]
            ):
                return "status", 0.8
        ...
        if any(
            word in text
            for word in ["like", "containing", "matching", "similar", "find", "search"]
        ):
            return "list", 0.7
    ...
    def _extract_parameters(
        self, user_input: str, tokens: List[str], command: str
    ) -> Dict[str, Any]:
        ...
        for pattern in status_host_patterns:
            ...
            if status_host_match:
                ...
                if (
                    not self._is_service_keyword(hostname)
                    and hostname.lower() not in action_words
                ):
                    parameters["host_name"] = self._apply_fuzzy_hostname_matching(
                        hostname
                    )
                    break

        # Extract hostname from "list/show services on hostname" patterns
        list_services_pattern = (
            r"(?:list|show|get|find)\s+(?:all\s+)?services\s+(?:on|for)\s+(\w+)"
        )
        ...
        service_pattern = (
            r"(?:acknowledge|ack|downtime|check|status)\s+(.+?)\s+(?:on|for|of)\s+"
        )
        ...
        for pattern in contextual_patterns:
            if re.search(pattern, text, re.IGNORECASE):
                if self.conversation_context["last_host"] and not parameters.get(
                    "host_name"
                ):
        ...
        if parameters.get("service_description"):
            self.conversation_context["last_service"] = parameters[
                "service_description"
            ]
    ...
    def get_command_type(
        self, command: str, parameters: dict = None, original_input: str = ""
    ) -> str:
        ...
        if original_input:
            ...
            is_service_listing = "service" in original_lower and any(
                word in original_lower for word in ["show", "list", "get", "find"]
            )
            is_host_listing = "host" in original_lower and any(
                word in original_lower for word in ["show", "list", "get", "find"]
            )
        ...
        if original_input:
            ...
            if "status" not in original_lower and any(
                keyword in original_lower for keyword in ["service", "services"]
            ):
                return "service"
        ...
        if command == "list":
            ...
            if original_input:
                ...
                if "service" in original_lower:
                    return "service"
                elif "host" in original_lower or not any(
                    s in original_lower for s in ["service"]
                ):
                    return "host"
